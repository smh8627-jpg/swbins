...
(function (global) {
  ...
  var REG = {
    ...
    /* **벼는 아직 못 세웠다.** `Rice_4.glb` 는 받아 뒀고 파일도 로드도 정상인데
       (three 로 직접 재니 키 0.95m · 메시 둘), 이 인스턴싱 길에 얹으면 화면에
       한 포기도 안 선다. 계획은 나온다(진단이 값으로 붙들고 있다 — 논 격자마다
       벼가 여럿). 등급을 HIGH 로 못박아도, 창고를 2000 자리로 키워도 그대로다.
       원인을 못 찾아 **표에서 뺐다** — 안 보이는 죽은 길을 남기지 않으려는 것이다.
       논바닥과 두렁은 코드가 그리는 것이 맞다(물 댄 낯이라 모델보다 낫다). */
  };
