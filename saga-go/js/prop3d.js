...
(function (global) {
  ...
  var REG = {
    ...
    /* **벼는 아직 못 세운다 — 모델 탓이 아니다.** 2026-08-28 에 끝까지 따라가
       알아낸 것을 적어 둔다(다음에 같은 자리를 파지 않도록):
       · `Rice_4.glb` 자체는 멀쩡하다 — three 로 직접 로드해 조각 둘(정점 2482 ·
         1472), 키 1.000, 폭 0.797 로 정규화되고 **실제로 그려진다**(픽셀을 세었다)
       · 계획도 정상이다 — `propPlan` 이 논 격자마다 벼를 낸다(진단이 붙들었다)
       · 인스턴스 창고에도 **들어간다** — `world3d.instReport('Rice')` 가
         `Rice_4.glb#0=78/78` 을 준다. 메시 상태도 정상(보임·재질·정점·부모)
       · **그런데 화면에 한 포기도 안 선다.** 키를 20m 로 키워도, 격자 한가운데에
         고정해도, 그 자리에 **나무를 대신 넣어도** 안 선다 — 즉 모델이 아니라
         `rice` 라는 이름으로 도는 이 길 어딘가다
       · 실마리: 창고 첫 자리의 좌표가 **(9, 0.2, 0)** 으로 찍힌다. `p.x` 를 0 으로
         고정했으므로 격자 중심(24 의 배수 + 24)이어야 하는데 그 값이 아니다.
         **좌표가 엉뚱한 데로 간다** — 여기서부터 다시 파면 된다
       논바닥과 두렁은 어차피 코드가 그리는 것이 맞다(물 댄 낯이라 모델보다 낫다). */
  };
