...
(function (global) {
  ...
  function levelAt(gx, gy) {
    ...
    var j = JITTER[k];
    if (j) {
      /* 2026-09-09 — `hash2`를 잔무늬에 그대로 쓰면 칸마다 완전히 독립이라
         (소금·후추 노이즈) 언덕이 한 칸씩 뚝뚝 끊겨 솟았다 꺼졌다 해
         "바닥이 바둑판 같다"로 보였다 — `world.js`의 `terrainNoise`(지형
         종류가 같은 이유로 바둑판져 보이던 것을 고친 자리, 2026-09-08)와
         똑같은 결함이라 같은 처방(2옥타브 `noise2`)을 그대로 옮긴다.
         `noise2`는 0~1을 주므로 ±j로 펴려면 *2-1. */
      var jn = core().noise2(gx * 37 + 11, gy * 53 + 7, 5) * 0.7 +
