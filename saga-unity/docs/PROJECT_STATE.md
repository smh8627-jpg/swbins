...
마지막 갱신: 2026-09-24 (서른일곱 세션째 — **PLAN 107장 GO 원신 기준: ① 들판 전투 + ② 이동 완료**. ②: 산 = 보이는 절벽 고원(12~22m·테두리 30~38m), 강 = 헤엄치는 깊은 물, 점프·등반·넘어오르기·활공·수영 + Maria 전용 클립 5. 헤드리스 통과, 실기 확인은 전. 경위는 HISTORY 날짜 grep).
...
|---|---|---|---|---|---|
| GO | `TestVillage` | 완료 — 도적의 습격(이동·촌장·상인·나그네·조우·전투·등용·EXP·장비·루트·저장 v13) | 동물 Group·나그네·은닉 보물·산신당·행운 돌탑·동굴 유물·채집 · 101-2 ④⑦③: 일과판·승급 3택·75초 토벌 · ①⑥⑧: 봉수대·인연·패배 비용·회수 · ②: 사당 시련(파도 3·인장 조각) · **107 ①② (2026-09-24)** 들판 전투(`Combat/` — 적 15·원소 반응·교체) + 이동(절벽 고원·깊은 강·점프·등반·활공·수영) | Player·주요 Enemy·Environment·Building 전부 GLB/PBR, Props는 fence·fenceGate만(lantern·stall 보류), Rocks/Vegetation은 procgen 트라이플레이너(102-4) | 전부 붙음. 목표판/세션카드(A·B). 101-3 C·F·G 전부 완료 |
...
0. **PLAN 107장 GO 원신 기준이 최우선**(사용자 2026-09-24 "지금 바로"). ①② 완료 → 다음 **③ 지역 지도**: 고원이 네모 기둥뿐이라 봉우리·경사, 멀리 보이는 랜드마크(높은 곳 목표), 순간이동 지점(역참), M 지도 화면(지역 이름·발견한 곳·순간이동), 지역마다 바이옴. 그다음 ④ 상자 ⑤ 원소 적 ⑥ 동료 모델. DUNGEON 106 순서 5(점프·등반)는 GO `PlayerController` 이동 상태를 판별 복사하면 된다.
0-1. **107 ①② 남은 것**: 새 글자 `field.*`·`move.*` 가 `GoLocalization.T(키, 한국어)` 폴백 — ko/en 표(다섯 벌 md5)에 넣기. 동료 교체는 몸이 안 바뀌고 원소 빛깔만(⑥). 넘어오르기 전용 클립 없음(0.45s 코드 이동). 다른 PC 는 Maria 이동 클립 5개를 README 레시피로 받고 `Saga/Build Maria Traversal` 한 번.
...
| `PlaytestDungeonHeadless` | **3연속 OK(2026-09-24, 106-4 뒤)** — 새 `CheckNpcModels`(동행·마을 사람 3·포로 Kneel·행상 5·해골 6 Humanoid) + `CheckTemple`(능묘 + 106-3 컷) + `cut camera live/back`. 앞 분: `CheckLockOn`·`CheckEnemyTelegraph`. 같은 씬 `FloorProgression` OK |
| GO `PlaytestHeadless` | **3연속 OK(2026-09-24, 107 ② 뒤)** — `PlaytestGoTraversal`(점프 2.30m·등반 3.2m/s·활공 낙하 3.06/전진 10.2·수영 높이·강둑·익사 복귀·경계벽·적 칸 제한·전용 클립 파라미터) + `PlaytestGoFieldCombat`(반응 표·적 15·3타/콤보·처치 경험치·증발/과부하/감전·스킬/폭발·발견→예고→피격·회피 무적·교체·전멸 복귀·DuelGate·진짜 onClick) |
