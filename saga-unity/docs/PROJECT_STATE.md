...
마지막 갱신: 2026-09-19 (이어서 **PLAN 101-2 5.1 DUNGEON 축복 3택** — `BlessingState`(공/수/선 3축, `PerkState`와 같은 구조), 보스층마다 카드, SaveState v6. 웹판 원소·다중 스킬 시너지 대신 이 트랙 고유 강공격/회전베기 빌드를 강화하는 쪽으로 재해석 — 경위는 HISTORY grep. 컴파일 0 오류, DUNGEON 헤드리스 재검증 OK). 그 앞: GO 101-2 ④⑦③ 완료. 그 앞: REALM 51장 확장.
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류(우물·상자·성소·행상)·회피·강공격·필드(방 2+복도)·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드(회전베기)·도감·보석/영웅 상태 · **101-2 5.1 축복 3택 완료(2026-09-19)** | Player·잡졸(황건적)·미니보스/두목·Environment·Building | 전부 붙음(SFX 실클립 통일). **목표판/세션카드(101-2 A·B)**. **101-3 C·F·G 전부 완료(2026-09-17)** |
...
1. **PLAN 101-2 이어서** — GO 첫 세 항목(④⑦③) 완료, 다음은 ①②⑥⑧ 중 사용자 결정 대기(⑤는 모바일 빌드 뒤). DUNGEON 5.1(축복 3택) 완료(2026-09-19) — 다음은 5.2(유품). 다른 두 판의 101-2 둘째·셋째 항목도 착수 가능.
...
| `PlaytestDungeonHeadless` | **재검증 OK**(2026-09-19, 축복 3택 뒤 — 10 frames, no errors) |
| `PlaytestDungeonFloorProgression` | **재검증 OK**(2026-09-19, 축복 3택 뒤 — 12 room advances, floor 4, no errors, 보스층 진입 시 카드 트리거 경로 포함) |
...
- DUNGEON: 카메라 각도, 아홉 슬라이스, 목표판/세션카드, hitstop·타격VFX·유품마커·레벨업줌·무기소켓·지형데칼 체감(101-3 전체), **축복 3택 UI·체감(101-2 5.1, 2026-09-19 신규 — 첫 실기 확인)**
