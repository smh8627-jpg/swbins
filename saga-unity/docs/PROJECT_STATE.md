...
마지막 갱신: 2026-09-24 (마흔네 세션째 — **PLAN 106 FF·젤다 방향 여섯 순서 전부 코드 완료**: 순서 6 FF 확장 = 동료 파티(무사 도발·술사 치유·ATB 게이지) + 소환수 "바위 거신"(Timeline 컷). GO 107장도 전부 코드 완료(식생 바이옴 보류). 헤드리스 통과, 실기 확인은 전. 경위는 HISTORY 날짜 grep).
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류·회피·강공격·필드·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드·도감·보석/영웅 상태 · **101-2 전부 완료** · **106-5 탐험** 점프 F·담쟁이 등반·옛 감시탑 뜰 · **106-6 FF 확장(2026-09-24)** 무사·술사 파티, 명령 1·2, 소환 V | Player·잡졸·미니보스/두목·Environment·Building · 103-1 방 셸 마모 3단 | 전부 붙음. 목표판/세션카드. 101-3 C·F·G 전부 완료 |
...
0. **PLAN 106 FF·젤다 방향(DUNGEON)** — 순서 1~6 전부 코드 완료. 다음은 사용자와 정할 것: 106 결을 GO·STORY 로 옮기기(두목 등장 컷·파티·소환, 판별 복사) 또는 106-6 남은 것(술사·무사 전용 클립, 소환수 전용 모델·발광 재질, 동료 성장). GO 107장은 전부 코드 완료(식생 바이옴 보류).
0-1. **107·106-6 남은 것**: 새 글자 GO `field.*`·`move.*`·`chest.*`·`map.*`·`field.foe.*`·`field.st.*`, DUNGEON `party.*`·`action.taunt/heal/summon`·`cut.summon_*` 가 `T(키, 한국어)` 폴백 — ko/en 표(다섯 벌 md5)에 넣기. 동료 몸은 Maria.controller 리타깃(전용 클립 없음), 무기는 주인공 손에만. 넘어오르기 전용 클립 없음(0.45s 코드 이동). 다른 PC 는 Maria 이동 클립 5개를 README 레시피로 받고 `Saga/Build Maria Traversal` 한 번.
...
| `PlaytestDungeonHeadless` | **3연속 OK(2026-09-24, 106-6 뒤)** — `PlaytestDungeonParty`(게이지·도발·쓰러짐·치유·빛살·소환 컷/내리치기/넘김)·`PlaytestDungeonExplore`·`CheckNpcModels`·`CheckTemple`(+컷)·`CheckLockOn`·`CheckEnemyTelegraph`. 같은 씬 `FloorProgression` OK |
| GO `PlaytestHeadless` | **3연속 OK(2026-09-24, 107-3 경사·바이옴 뒤)** — `PlaytestGo` SlopesBiome·PartyBodies·ElementalFoe·Treasure·WorldMap(세이브 v14/v13)·Traversal·FieldCombat (항목은 각 파일 요약 주석) |
...
- DUNGEON: **파티·소환(106-6) — 무사·술사가 따라오는 모양(술사 Maria 동작 리타깃이 어색하지 않은지), 게이지 차는 속도(명령 약 8타·소환 약 34타), 도발 고리·"이쪽이다" 체감, 무사 눕는 자세, 치유 빛, 빛살 빛깔, 파티 줄 가독성·버튼 흐림, 소환 컷 5초 길이·거신 크기(3.2배)·솟아오름·내리치기 손맛·한 방 세기(평타×15), 카메라가 벽에 안 박히는지** · **탐험(106-5) — F 점프 1.3m 손맛, 담쟁이 탑 오르기 속도·클립이 벽에 붙는지, 넘어오르기, 계단 돌·2m 틈 난이도, 탑 불빛이 Room2 에서 보이는지, 막이가 어색하지 않은지** · **사실 모델 NPC(동행 기사 걷기↔달리기 발 미끄럼·마을 사람·포로 무릎→기쁨·행상 서는 자리·해골 파수꾼 크기/칠)**, **컷 셋(능묘 도착 4.5s·상자 2.6s·능묘지기 등장 5.2s) 길이·레터박스·"아무 키로 넘김"·상자 카메라가 벽에 안 박히는지·손떨림 세기**, **잊힌 능묘 한 바퀴(블록·벽력탄 심지·능묘지기 사이클·열쇠 줄)**, **락온(카메라 추적·예고 0.5s·완벽 회피 반격·옆걸음 발 미끄럼)**, 옛 VS·101 항목(목록은 HISTORY), **폰에서 공격·강공격·회전베기·회피·저장·설정·축복 버튼(2026-09-23 고침)**
