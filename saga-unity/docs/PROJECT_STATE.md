...
마지막 갱신: 2026-09-19 (이어서 **PLAN 101-2 5.4 DUNGEON 월드 보스** — 웹판 "15분 실시간 슬롯+마을 필드 스폰"은 허브 마을이 없어 안 옮기고, 기존 "층 끝 두목"을 75초 제한 전투로 승격(`DungeonEnemy.isWorldBoss`) — 타이머(`PlayerHud`), 부위 3(GO ③ 문턱 재사용) 파괴·완파 보너스, 시간 초과 도망(보상 30%). 저스트 회피는 스코프 제외. 경위는 HISTORY grep. 컴파일 0 오류, 헤드리스 재검증 OK). DUNGEON 101-2(5.8~5.4) 전부 완료.
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류(우물·상자·성소·행상)·회피·강공격·필드(방 2+복도)·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드(회전베기)·도감·보석/영웅 상태 · **101-2 5.1~5.4 완료** | Player·잡졸(황건적)·미니보스/두목·Environment·Building | 전부 붙음(SFX 실클립 통일). **목표판/세션카드(101-2 A·B)**. **101-3 C·F·G 전부 완료(2026-09-17)** |
...
1. **PLAN 101-2 이어서** — GO 첫 세 항목(④⑦③) 완료, 다음은 ①②⑥⑧ 중 사용자 결정 대기(⑤는 모바일 빌드 뒤). DUNGEON 5.1~5.4 완료 — 다음은 5.5. 다른 두 판의 101-2 항목도 착수 가능.
...
| `PlaytestDungeonHeadless` | **재검증 OK**(2026-09-19, 5.1~5.4 뒤 — 10 frames, no errors) |
...
- DUNGEON: 카메라 각도, 아홉 슬라이스, 목표판/세션카드, hitstop·타격VFX·레벨업줌·무기소켓·지형데칼 체감(101-3 전체), **축복·유품·부적 던전·월드 보스 체감(101-2 5.1~5.4, 신규 — 첫 실기 확인)**
