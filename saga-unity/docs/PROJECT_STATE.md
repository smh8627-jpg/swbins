...
마지막 갱신: 2026-09-24 (서른다섯 세션째 — **PLAN 106장 FF·젤다 순서 1~3 코드 완료 + 4 DUNGEON 완료**: ① 락온·예고·반격 ② 젤다식 던전 "잊힌 능묘" ③ Cinemachine·Timeline 컷 셋 ④ **NPC 사실 모델**(동행 Paladin·마을 사람 Peasant 남녀·포로 무릎 꿇기→기쁨·행상·능묘 해골 파수꾼, `SetupNpcCharacterImports`). 헤드리스 통과, 창 모드 실기 확인은 전. 경위는 HISTORY 날짜 grep).
...
0. **PLAN 106장 FF·젤다 순서가 최우선**(무예·전직 같은 수치 확장은 멈춤). 순서 4는 DUNGEON 완료(106-4 끝 "남은 것": 능묘지기 전용 모델 여부·층 두목 변형·다른 판 NPC) → 다음 **순서 5 탐험**: 점프·기어오르기·높은 곳 랜드마크(DUNGEON). Mixamo 는 `tools/mixamo_automation`(`--character`·`--tpose`·`--inplace`, 로그인만 사람 몫).
