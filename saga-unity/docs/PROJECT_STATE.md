...
마지막 갱신: 2026-09-19 (이어서 **REALM 51장 27차 확장** — 대하→오익산리, 16~26차는 같은 날짜대). **Maria/Abe/Brute Mixamo 실자산 확보 + GUI 육안 확인**은 그 앞 세션. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 47, 성 50**(51장 27차 — 교주 사슬 남해→{창오→울림→교지→구진→일남→상림→{전충→비경, 노용}, 합포}, 남중 사슬 주제→건녕→{월수,장가,운남→영창→신독→{건타라→계빈, 대하→오익산리}}(상림·건녕·신독 셋 다 목표 둘 이상)) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
## REALM 51장 16~27차 — 국경 확장 (2026-09-18~19, 완료·요약만)

"성 하나당 목표 하나" 제약을 풀고(`RealmEnemyCity.TargetsFrom()` 복수 반환, `RealmCommandUi` 공격·계략 고르기 패널) 사슬을 {전충→비경, 노용}·{건타라→계빈, 대하→오익산리}까지 뻗었다(적국 47·성 50, 위 표 참고). 27차: 대하→오익산리(더 뻗는 LINKS 없어 이 갈래 끝). 경위는 `docs/HISTORY.md` 2026-09-18~19 grep. 남은 후보: 신독 잎사귀 마게타/사위(고르면 신독 갈래 전부 닫힘).

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널·17~27차 신규 성 22개 포함). 사용자 몫.
...
3. **REALM 28차 후보** — 신독 잎사귀 마게타/사위 중 하나. 사용자와 방향 상의.
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI(2026-09-18)·51장 17~27차 확장(2026-09-18~19) 모두 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| 검증 | 결과 |
|---|---|
| `-batchmode -nographics -quit` 컴파일(`tools/unity-batch.sh` 경유) | exit 0, 오류 0(REALM 27차 확장 뒤 재확인) |
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-19, 51장 16~27차 확장 + 계략 고르기 UI 뒤 — 장안·건녕(목표 3)·상림·신독(목표 2씩)·남해 다중 목표 공격·계략 고르기 패널·잘못된 목표 거절·스물두 개 신규 성(비경·건타라·노용·계빈·대하·오익산리 포함) 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안·남해·건녕(목표 3개)·상림·신독(목표 2개씩) 등) 실제 조작감(51장 16~27차 + 계략 고르기 UI, 2026-09-18~19 신규 — 이게 첫 실기 확인)**
