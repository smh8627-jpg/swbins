...
마지막 갱신: 2026-09-18 (이어서 **REALM 51장 17차 확장**(남해→창오·주제→건녕) + **REALM 계략 고르기 UI 완료** — 아래 두 절 참고. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 29, 성 32**(51장 17차 — 16차가 국경 성 3곳(장안·장사·강주)에 연 둘째 목표(천수·남해·주제)에 이어, 17차가 그중 둘을 한 단계 더 뻗었다: 남해→창오(교주)·주제→건녕(남중)) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
- ~~**알려진 틈**: `Plot()`(계략)은 아직 고르기 UI가 없다~~ — **2026-09-18 해소**.

## REALM 51장 17차 — 교주·남중 한 단계 더 (2026-09-18, 같은 날 "사가유니티 이어해")

16차가 남긴 다음 후보 둘을 이었다: **남해→창오**(교주 사슬, 원작 LINKS nanhai-cangwu)·**주제→건녕**(남중 사슬, 원작 LINKS zhuti-jianning). 둘 다 16차의 "형제 가지"(같은 출진 성의 둘째 목표)가 아니라 정상적인 한 단계 더 깊은 자식이라 train은 옛 규칙(부모 train+15)을 그대로 썼다. 남해의 다른 이웃(합포)·건녕의 다른 이웃(월수·장가·운남)은 다음 확장 후보로 남김.
- **실제로 걸린 함정**: `RealmEnemyCity.cs`에만 새 성을 추가하고 `RealmCityData.cs`(플레이 가능 성의 agri/comm/pop/좌표 정의)에는 안 추가해 첫 검증에서 실패했다 — `RealmCityState.AbsorbCity()`가 `RealmCityData.Get(cityId)==null`이면 **조용히 아무것도 안 하고 return**한다(에러도 안 던짐). 함락 자체(`RealmWarState.Attack()`)는 `Ok=true, Won=true`로 성공해 버려서, 헤드리스 검증이 `RealmCityState.ActiveCityIds.Contains()`로 편입 여부를 따로 확인하는 덕에 잡혔다. **새 REALM 성을 추가할 땐 항상 두 파일(`RealmEnemyCity.cs`+`RealmCityData.cs`) 다 같이 고칠 것** — 하나만 고치고 넘어가기 쉽다.
- `PlaytestRealmSlice.cs`에 `Phase.AttackCangwu`/`Phase.AttackJianning` 신설(AttackZhuti 다음, QuizCorrect 앞) — 둘 다 목표가 하나뿐인 성이라 `enemyId` 생략(기존 단순 체인 스텝과 같은 결).

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널·17차 신규 성 둘도 포함). 사용자 몫.
2. **PLAN 104-1 ⑤·102-4** — `Assets/Art/*_candidates` 정리, 105 Q1 결정 대기.
3. **REALM 교주·남중 더 뻗기(18차 후보)** — 남해→합포, 창오→울림, 건녕→월수/장가/운남(허브라 갈래 여럿). 사용자와 방향 상의(또는 그대로 "계속 확장" 기조 유지, 지금까지처럼 물으면 이어감).
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI·51장 17차 확장 둘 다 2026-09-18에 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-18, 51장 16·17차 확장 + 계략 고르기 UI 뒤 — 장안 다중 목표(2개) 공격·계략 고르기 패널(각 2/4버튼)·잘못된 목표 거절·천수/남해/주제/창오/건녕 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안 등 목표 둘인 성) 실제 조작감(51장 16·17차 + 계략 고르기 UI, 2026-09-18 신규 — 이게 첫 실기 확인)**
