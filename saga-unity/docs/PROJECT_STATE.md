...
마지막 갱신: 2026-09-17 (REALM 51장 15차 + Localization 잔여 감사 + PLAN 101-2 A·B **다섯 판 전부 이식 완료** + **105 Q-U5 DoF 토글** + **DUNGEON·STORY hitstop/shake/flash/popup(101-3 C, "손맛 2차"·"손맛 표준") 구현 완료** + 다섯 게임 컴파일·씬 재생성·헤드리스 3연속 검증 완료, STORY 애니메이터 버그 1건 수정).
...
| STORY | `TestField` | 완료 — 2.5D 횡스크롤(Z 고정)·잡졸 10·두목·사명 2·볼트·로프 | 척후병 NPC·사건·관계·선택(51장 완결)·전직(Lv.10, 무사/궁수/협객/방사) | 척후병 실제 모델 | 전부 붙음. **목표판/세션카드(101-2 A·B)**. **hitstop/shake/flash/popup(101-3 C "손맛 표준", 2026-09-17)** — 크리티컬 전역 슬로모(웹판 원문)는 그대로 유지. `PlayAttackAnim()` animator? 버그 수정 |
...
- **DUNGEON·STORY hitstop/shake/flash/popup 구현 완료(101-3 C, PLAN 5.8/5-7 "손맛 2차/표준")** — DUNGEON `PlayerCombat.ApplyHitstop()`(평타 70ms·강공격 120ms, 가해자+피해자 Animator.speed=0). STORY `StoryCombat.ApplyHitFreeze()`(가해자만 — 이 판은 적 Animator가 없다) + 신규 `StoryEnemy` flash/popup(`DamagePopup.cs` 신설) + `StoryCameraFollow.Shake()`(신규, `Instance` 싱글턴) — 웹판 원문 크리티컬 전역 슬로모(`TriggerHitstop`/`Time.timeScale`)는 손 안 대고 별개로 공존. `PlaytestDungeonHeadless.CheckHitstop()`·`PlaytestStorySlice.CheckHitFeedback()`이 공격 직후 실제 Animator.speed==0/shake 타이머로 검증(3연속 전부 OK). VFX·데칼·장비 소켓 등 101-3 나머지 C·G 항목, GO 쪽 hitstop은 아직.
...
| `PlaytestStorySlice`(전직·언어 전환·save/load, hit feedback(shake+hitstop) 검증 포함) | 첫 3연속 전부 실패(animator 버그) → 수정 후 3연속 OK, DoF·hit-feel 반영 뒤도 3연속 OK(이 씬 player Animator는 null이라 hitstop 자체 검증은 스킵, shake는 확인됨) |
...
- STORY: 두목 크기·타격감(반격 없음이 샌드백처럼 느껴지는지), 사건·관계·선택 흐름, 전직 팝업, 척후병 모델, **목표판 3줄 화면 배치·세션카드 실제 등장**, **hitstop/shake/flash/popup 체감(크리티컬 슬로모와 안 겹치는지 — 101-3 C)**
