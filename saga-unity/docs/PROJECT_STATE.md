...
마지막 갱신: 2026-09-17 (PLAN 101-2 A·B **다섯 판 전부 이식 완료** + **105 Q-U5 DoF 토글** + **101-3 C 전부(hitstop/shake/flash/popup/타격 VFX) GO·DUNGEON·STORY 완료** + **101-3 F 죽음(`LootMarker`) DUNGEON 구현 완료** + 다섯 게임 컴파일·씬 재생성·헤드리스 3연속 검증 완료).
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류(우물·상자·성소·행상)·회피·강공격·필드(방 2+복도)·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드(회전베기)·도감·보석/영웅 상태 | Player·잡졸(황건적)·미니보스/두목·Environment·Building | 전부 붙음(SFX 실클립 통일). **목표판/세션카드(101-2 A·B, 2026-09-17 검증 완료)**. **hitstop·타격 VFX·죽음 표식(101-3 C·F, 2026-09-17)** |
...
- **101-3 F 죽음(`LootMarker`) DUNGEON 구현 완료** — `DungeonEnemy.Die()`가 기존 즉시 보상(경험치·돈·장비·보석) 뒤에 호출, 회수 반경 2m(표 그대로)·주워도 추가 보상 없음(이미 준 보상의 시각적 잔향)·12초 안 주우면 소멸. STORY는 적 쪽에 Animator가 없어 범위 밖. 남은 101-3: G 장비 소켓·G 데칼·G 성장 연출뿐.
...
3. ~~PLAN 101-3 C 전부·F 죽음~~ — hitstop·shake·flash·popup·타격 VFX(GO·DUNGEON·STORY)·유품 마커(DUNGEON) 전부 완료(2026-09-17). 남은 101-3은 **G 장비 가시화(소켓)·G 지형 반응(데칼)·G 성장 연출(Timeline)** 셋뿐 — 셋 다 새 렌더 기능/데이터 스키마가 필요해 사용자와 방향 먼저 확인.
...
| 검증 | 결과 |
|---|---|
| `-batchmode -nographics -quit` 컴파일 | exit 0, 오류 0(라이선스 토큰 경고만, LootMarker 추가 뒤 재확인) |
...
| `PlaytestDungeonHeadless`(hitstop·타격 VFX·`CheckLootMarker`(유품 마커 스폰+자연 프레임 픽업) 실제 검증 포함) | 3연속 OK(LootMarker 추가 뒤 재검증). 하위 슬라이스(`FloorProgression`·`FieldAmbush`·`Shortcut`·`Town2`·`Towns34`) 는 DoF 반영 시점에 1회씩 재검증 — 회귀 없음(이후 추가된 HitSpark/LootMarker는 아직 재확인 안 함, TakeDamage 경로만 공유해 위험 낮다고 판단) |
...
- DUNGEON: 카메라 각도·공격 버튼 vs CameraRig 드래그 겹침, 아홉 슬라이스 전부(무리·엘리트/보스·방 종류·회피·필드·강공격·행상·동행·GLB), 마을 넷·층 진행·매복·구출·수수께끼·회전베기, 사실적 Player/Enemy/Boss 톤, 무드 유지 판단, **목표판 3줄 화면 배치(미니맵과 안 겹치는지)·세션카드 실제 등장**, **hitstop 체감(70/120ms, 과하지 않은지 — 101-3 C)**, **타격 VFX·유품 마커 색·크기 체감(둘 다 `Sprites/Default`·기본 Lit 이라 텍스처 없이 밋밋해 보일 수 있음 — 101-3 C·F)**
