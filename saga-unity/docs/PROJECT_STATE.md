...
마지막 갱신: 2026-09-19 (이어서 **REALM 51장 29차 확장** — 신독 넷째 자식 사위, 신독 갈래 전부 닫힘. 16~28차는 같은 날짜대). **Maria/Abe/Brute Mixamo 실자산 확보 + GUI 육안 확인**은 그 앞 세션. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 49, 성 52**(51장 29차 — 교주 사슬 남해→{창오→울림→교지→구진→일남→상림→{전충→비경, 노용}, 합포}, 남중 사슬 주제→건녕→{월수,장가,운남→영창→신독→{건타라→계빈, 대하→오익산리, 마게타, 사위}}(신독 목표 넷, 갈래 전부 닫힘)) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
## REALM 51장 16~29차 — 국경 확장 (2026-09-18~19, 완료·요약만)

"성 하나당 목표 하나" 제약을 풀고(`RealmEnemyCity.TargetsFrom()` 복수 반환, `RealmCommandUi` 공격·계략 고르기 패널) 신독을 {건타라→계빈, 대하→오익산리, 마게타, 사위}까지 뻗었다(적국 49·성 52). 29차: 신독 넷째 자식 사위 — 신독의 원작 LINKS 넷을 전부 채워 갈래가 닫혔다. 경위는 `docs/HISTORY.md` 2026-09-18~19 grep. 남은 후보(원작 LINKS 재확인): 노용→주오, 전충의 다른 이웃 서권/구속(잎사귀).

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널·17~29차 신규 성 24개 포함). 사용자 몫.
...
3. **REALM 30차 후보** — 노용→주오, 또는 전충의 다른 이웃 서권/구속. 사용자와 방향 상의.
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI(2026-09-18)·51장 17~29차 확장(2026-09-18~19) 모두 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| 검증 | 결과 |
|---|---|
| `-batchmode -nographics -quit` 컴파일(`tools/unity-batch.sh` 경유) | exit 0, 오류 0(REALM 29차 확장 뒤 재확인) |
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-19, 51장 16~29차 확장 + 계략 고르기 UI 뒤 — 장안·건녕·상림·남해 다중 목표 공격·신독(목표 4, 신규 최다)·계략 고르기 패널·잘못된 목표 거절·스물네 개 신규 성(비경·건타라·노용·계빈·대하·오익산리·마게타·사위 포함) 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안·남해·건녕(목표 3)·신독(목표 4)·상림(목표 2) 등) 실제 조작감(51장 16~29차 + 계략 고르기 UI, 2026-09-18~19 신규 — 이게 첫 실기 확인)**
