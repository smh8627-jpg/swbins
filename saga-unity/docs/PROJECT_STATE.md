...
마지막 갱신: 2026-09-17 (REALM 51장 14차, 상군→삭방 — 이 PC 에 Unity 없어 컴파일 미검증).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 23, 성 26**(51장 14차, 사슬: 허창→소패→하비→수춘→여남→강하→양양→강릉→장사→시상→건업→회계(막다른 끝) / 복양→정도→업→진양→운중→상군→삭방(막북 끝 한 칸 남음, 오원) / 진류→낙양→장안→한중→성도→강주→영안(막다른 끝)) | 도시 Environment/Building | 전부 붙음. `RealmCommandUi` 직렬화 버그(2026-09-15) 수정·저장 버튼 신설 |
...
- **REALM 51장 12~14차 확장(이번 세션)** — 진양→운중→상군→삭방(원작 LINKS: jinyang-yunzhong, yunzhong-shangjun, shangjun-shuofang). `data-city.js`를 jinyang·yongan으로 전체 grep해 재확인 — yongan은 진짜 막다른 가지(이웃 둘 다 이미 우리 성) 그대로였지만, **jinyang은 4차 확장 때 판정이 틀렸었다**: 화북 본토 LINKS 구역만 보고 놓쳤던 `['jinyang', 'yunzhong']`(542행, "막북" 창작 확장 구역)이 따로 있었다. 매 단계 잎사귀(안문·정양·북지 — 다른 LINKS 없음)는 건너뛰고 계속 뻗을 수 있는 쪽(상군→삭방)만 골랐다. `RealmCityData.cs`·`RealmEnemyCity.cs`(`YunzhongId`·`ShangjunId`·`ShuofangId` 상수·`AllIds`·`Catalog` 항목 — wall/agri/comm/pop 원본 그대로, troops=wall×0.23 반올림, train=+15씩 90·105·120)·`PlaytestRealmSlice.cs`(`Phase.AttackYunzhong`·`AttackShangjun`·`AttackShuofang` 신설, `AttackChainStep` 체인 연결)만 고쳤다 — 지난 확장들과 같은 3파일 범위. 적국 20→23, 성 23→26. 삭방 자신은 오원(wuyuan) 하나로만 더 뻗을 수 있고 그러면 이 복양발 막북 가지도 끝난다(오원의 다른 LINKS 없음, 잎사귀). `RealmWorldMap`·`RealmCityState` 등은 `AllIds` 순회라 자동 반영 확인함.
- **테스트 상태 표(아래)는 전부 이번 세션 편집 이전 결과다.** 이번 세션에 고친 파일(12~14차 3개 + 이전 세션 미검증분) 전부 재검증 전.

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **컴파일·재검증(최우선)** — 누적 미검증분(104-1 9개 + 101-2 신규 5개 + REALM 51장 10~14차 9개) 전부. Unity 에디터 있는 세션에서: 배치 컴파일 → `BuildTestXxxScene`(GO/DUNGEON/FOREST/STORY) + `BuildTestCityScene`(REALM) 재생성 → `PlaytestHeadless`·`PlaytestDungeonHeadless`·`PlaytestForestHeadless`·`PlaytestStorySlice`·`PlaytestRealmSlice` 3연속. 씬을 안 다시 지으면 [SerializeField] 승격·GoalBoard 배선·건업·회계·운중·상군·삭방 사슬 전부 실제로 검증되는 게 없다.
...
6. **REALM 51장 다음 확장(마지막 한 칸)** — 세 사슬 중 허창(→회계)·진류(→영안)는 확정된 막다른 끝. 복양 사슬은 삭방(shuofang)→오원(wuyuan) 한 칸만 남았고, 오원엔 다른 LINKS가 없어(잎사귀) 그 확장을 끝으로 세 사슬 전부 막다른 끝이 된다. 이번 세션에 안 고른 잎사귀들(안문·정양·북지)은 이미 보류로 남아 있다. 성을 "막다른 가지"로 적기 전엔 반드시 파일 전체를 그 id로 grep해 뒤쪽 확장 구역(한국·일본·교주·서역·남중·천축·막북·균열·폐허·묘역) LINKS까지 다 봤는지 확인할 것 — 4차 확장 때 jinyang을 앞쪽(462~490행) 본토 구역만 보고 오판한 전례가 있다. 세 사슬이 다 막히면 51장은 다른 시작 성 조합이나 새 갈래(정복·외교 등, RealmCityData.cs 클래스 주석 "제외" 참고 — 범위 재검토 필요)를 사용자와 상의해야 한다.
