...
마지막 갱신: 2026-09-24 (마흔다섯 세션째 — **PLAN 106 여섯 순서 전부 코드 완료** + 106-7 층 두목·살수 등장 컷. GO 107장도 전부 코드 완료(식생 바이옴 보류). 헤드리스 통과, 실기 확인은 전. 경위는 HISTORY 날짜 grep).
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류·회피·강공격·필드·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드·도감·보석/영웅 상태 · **101-2 전부 완료** · **106-5 탐험** 점프 F·담쟁이 등반·옛 감시탑 뜰 · **106-6** 무사·술사 파티, 명령 1·2, 소환 V · **106-7** 층 두목 등장 컷 | Player·잡졸·미니보스/두목·Environment·Building · 103-1 방 셸 마모 3단 | 전부 붙음. 목표판/세션카드. 101-3 C·F·G 전부 완료 |
...
0. **PLAN 106(DUNGEON)** — 순서 1~6 + 106-7(층 두목 컷) 코드 완료. 다음 후보: 106 결을 STORY·GO 로 옮기기(두목 등장 컷부터, 판별 복사) 또는 106-6 남은 것(전용 클립·소환수 모델). GO 107장은 전부 코드 완료(식생 바이옴 보류).
...
| `PlaytestDungeonHeadless` | **3연속 OK(2026-09-24, 106-6 뒤)** — `PlaytestDungeonBossIntro`·`PlaytestDungeonParty`·`PlaytestDungeonExplore`·`CheckNpcModels`·`CheckTemple`(+컷)·`CheckLockOn`·`CheckEnemyTelegraph`. 같은 씬 `FloorProgression` OK |
