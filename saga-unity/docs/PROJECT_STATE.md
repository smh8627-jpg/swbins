...
- **DUNGEON 오픈월드 확장 — 열 번째 세션에서 세 조각(Room5·6 층2 진입
  + Room7·8 층2 방 종류 추가 + Room9 층2 퍼즐방)을 끝냈다(위 "완료
  단계" 맨 위 세 항목 참고).** 방향은 "물리적 확장 계속" + "같은 층
  안에 방 종류 채우기"로 계속 확정 중. 층2에 아직 없는 웹판 kind:
  merchant·event(구출)·forage(채집), miniboss. 다음 조각 후보:
  (a) 위 남은 kind 중 하나로 Room10 잇기, (b) 이쯤에서 "층3"으로
  올려 층 개념 자체를 한 번 더 검증하기, (c) 사람이 먼저 Room5~9를
