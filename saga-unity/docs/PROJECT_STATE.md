...
마지막 갱신: 2026-09-19 (이어서 **REALM 51장 22차 확장**(일남→상림·운남→영창, 아래 절 참고) — 16~21차(국경 성 확장+계략 고르기 UI+두 사슬 계속 뻗기)는 2026-09-18~19. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 39, 성 42**(51장 22차 — 교주 사슬 남해→{창오→울림→교지→구진→일남→상림, 합포}, 남중 사슬 주제→건녕→{월수,장가,운남→영창}(건녕 목표 셋 — 형제 가지가 셋으로 늘어난 첫 사례)) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
## REALM 51장 16~22차 — 국경 확장 (2026-09-18~19, 16차는 Q-U2 결정)
...
17~22차: 두 사슬을 계속 뻗었다 — 교주: 남해→{창오→울림→교지→구진→일남→상림, 합포}, 남중: 주제→건녕→{월수,장가,운남→영창}. 대부분은 정상적인 한 단계 더 깊은 자식(train=부모+15)인데, 19·20·21차가 각각 건녕(장가)·남해(합포)·건녕(운남)에 형제 가지를 더해 **건녕은 이제 목표가 셋**(형제 가지가 셋으로 늘어난 첫 사례) — `TargetsFrom`/고르기 패널이 목표 개수와 무관하게 계속 잘 동작한다. 22차(일남→상림, 운남→영창)는 다시 정상적인 한 단계 더 깊은 자식으로 돌아갔다(형제 가지 아님). 새 목표를 추가할 때마다 그 도시를 출진지로 쓰는 기존 `AttackChainStep`은 `enemyId`를 명시로 바꿔야 한다(안 그러면 어느 목표인지 불명확) — 매 확장 라운드마다 반복 확인된 패턴. 월수는 잎사귀(더 이상 이웃 없음)라 그 갈래는 끝. 남은 후보: 상림→노용/전충, 영창→신독.
...
1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널·17~22차 신규 성 열넷도 포함). 사용자 몫.
2. **PLAN 104-1 ⑤·102-4** — `Assets/Art/*_candidates` 정리, 105 Q1 결정 대기.
3. **REALM 교주·남중 더 뻗기(23차 후보)** — 상림→노용/전충(둘 중 하나, 성 하나당 목표 하나 원칙이면), 영창→신독. 사용자와 방향 상의(또는 그대로 "계속 확장" 기조 유지, 지금까지처럼 물으면 이어감).
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI(2026-09-18)·51장 17~22차 확장(2026-09-18~19) 모두 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-19, 51장 16~22차 확장 + 계략 고르기 UI 뒤 — 장안·건녕(목표 3)·남해 다중 목표 공격·계략 고르기 패널·잘못된 목표 거절·14개 신규 성 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안·남해·건녕(목표 3개) 등) 실제 조작감(51장 16~22차 + 계략 고르기 UI, 2026-09-18~19 신규 — 이게 첫 실기 확인)**
