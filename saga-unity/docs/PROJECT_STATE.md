...
마지막 갱신: 2026-09-17 (REALM 51장 15차, 삭방→오원, 세 사슬 전부 마무리 — 이 PC 에 Unity 없어 컴파일 미검증).
...
- **REALM 51장 12~15차 확장(이번 세션) — 복양 사슬 마무리, 세 사슬 전부 막다른 끝** — 진양→운중→상군→삭방→오원(원작 LINKS: jinyang-yunzhong-shangjun-shuofang-wuyuan, "막북"). `data-city.js`를 jinyang·yongan으로 전체 grep해 재확인 — yongan은 진짜 막다른 가지(이웃 둘 다 이미 우리 성) 그대로였지만, **jinyang은 4차 확장 때 판정이 틀렸었다**: 화북 본토 LINKS 구역만 보고 놓쳤던 `['jinyang', 'yunzhong']`(542행, "막북" 창작 확장 구역)이 따로 있었다. 매 단계 잎사귀(안문·정양·북지 — 다른 LINKS 없음)는 건너뛰고 계속 뻗을 수 있는 쪽만 골라 오원(진짜 잎사귀)까지 갔다. `RealmCityData.cs`·`RealmEnemyCity.cs`(`YunzhongId`·`ShangjunId`·`ShuofangId`·`WuyuanId` 상수·`AllIds`·`Catalog` 항목 — wall/agri/comm/pop 원본 그대로, troops=wall×0.23 반올림, train=+15씩 90·105·120·135)·`PlaytestRealmSlice.cs`(`Phase.AttackYunzhong`·`AttackShangjun`·`AttackShuofang`·`AttackWuyuan` 신설, `AttackChainStep` 체인 연결)만 고쳤다 — 지난 확장들과 같은 3파일 범위. 적국 20→24, 성 23→27. **이제 세 사슬(허창·복양·진류) 전부 확정된 막다른 끝** — 51장을 더 늘리려면 이 세 시작 성 조합 밖(다른 시작 성, 또는 "정복·외교 제외" 결정 재검토)에서 찾아야 하고 사용자와 상의가 필요하다. `RealmWorldMap`·`RealmCityState` 등은 `AllIds` 순회라 자동 반영 확인함.
- **테스트 상태 표(아래)는 전부 이번 세션 편집 이전 결과다.** 이번 세션에 고친 파일(12~15차 3개 + 이전 세션 미검증분) 전부 재검증 전.

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **컴파일·재검증(최우선)** — 누적 미검증분(104-1 9개 + 101-2 신규 5개 + REALM 51장 10~15차 10개) 전부. Unity 에디터 있는 세션에서: 배치 컴파일 → `BuildTestXxxScene`(GO/DUNGEON/FOREST/STORY) + `BuildTestCityScene`(REALM) 재생성 → `PlaytestHeadless`·`PlaytestDungeonHeadless`·`PlaytestForestHeadless`·`PlaytestStorySlice`·`PlaytestRealmSlice` 3연속. 씬을 안 다시 지으면 [SerializeField] 승격·GoalBoard 배선·건업·회계·운중·상군·삭방·오원 사슬 전부 실제로 검증되는 게 없다.
...
6. **REALM 51장 더 늘리기(범위 재검토 필요, 사용자 상의)** — **세 사슬(허창·복양·진류) 전부 확정된 막다른 끝에 닿았다**(15차, 2026-09-17). 이 세 시작 성 조합만으로는 더 늘릴 자리가 없다 — 남은 안문·정양·북지 잎사귀는 각자 한 칸씩 짧게 붙일 수는 있으나 사슬을 "늘리는" 의미가 없다. 계속 늘리려면: (a) 새 시작 성/사슬을 아예 새로 여는 방법(원작 시나리오 194 조조군 성 셋 밖), 또는 (b) `RealmCityData.cs` 클래스 주석의 "정복·외교는 안 들인다(제외)" 결정을 재검토하는 방법 중 사용자와 상의해서 골라야 한다 — 감으로 먼저 코드를 고치지 말 것.
