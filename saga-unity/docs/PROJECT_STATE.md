...
마지막 갱신: 2026-09-19 (이어서 **PLAN 101-2 ④ GO 일과판 첫 실장** — 날짜 해시 오늘의 일과 3/도장 7=주간 보상, 2026-09-19 사용자 결정 "godot 실기 승인=검증"으로 착수). 그 앞: **REALM 51장 16~35차 확장** — 막북 북지·안문·정양까지 채워 교주·남중·복양 세 사슬 전부 완전히 닫힘. **Maria/Abe/Brute Mixamo 실자산 확보 + GUI 육안 확인**은 그 앞 세션. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
|---|---|---|---|---|---|
| GO | `TestVillage` | 완료 — 도적의 습격(이동·촌장·상인·나그네·조우·전투·등용·EXP·장비·루트·저장 v10) | 동물 Group·나그네·은닉 보물·산신당·행운 돌탑·동굴 유물·채집·오버월드 지도 · **101-2 ④ 일과판(2026-09-19)**: 걷기·도적 승리·늑대 토벌·성황당 기원 풀 4에서 날짜 해시 3택, 도장 7=주간 보상(`DailyTaskState`) | Player·주요 Enemy·Environment·Building·Props 전부 GLB/PBR | 전부 붙음. **목표판/세션카드(101-2 A·B)**. **101-3 C·F·G 전부 완료(2026-09-17)** |
...
1. **PLAN 101-2 이어서** — GO는 ④ 다음 순서 **⑦ 승급 3택**(공/수/보 3택)·**③ 75초 토벌**. 다른 네 판(DUNGEON/FOREST/STORY/REALM)의 101-2 둘째·셋째 항목(축복 3택·유품·관계 하트·직업 정체성·일기토 등, PLAN 101-2 표 참고)도 2026-09-19 결정으로 saga-web 검증 대기 없이 착수 가능.
...
| 검증 | 결과 |
|---|---|
| `-batchmode -nographics -quit` 컴파일(`tools/unity-batch.sh` 경유) | exit 0, 오류 0(GO 일과판 추가 뒤 재확인) |
| `PlaytestHeadless`(GO) | **3연속 OK**(2026-09-19, 일과판 추가 뒤 — 날짜 해시 결정성·진행/완료/도장·주간 보상·저장 왕복·v9 마이그레이션 새 검증 포함) |
...
- GO: 조우·전투·등용 손맛, 상점·퀘스트 대사 3단계, 은닉 보물·산신당·돌탑·유물, 채집, 목표판/세션카드, hitstop 체감, 유품 마커·무기 소켓·지형 데칼 실제로 보이는지(101-3 F·G), **일과판 3택 체감·도장 UI(101-2 ④, 2026-09-19 신규 — 이게 첫 실기 확인)**
