...
- **DUNGEON — PLAN.md(saga-dungeon 웹판) 챕터 순 심화, 첫 세 조각
  ...
  - **36장 퀘스트 시스템**(신규 `Data/QuestState.cs`) — 메인 퀘스트
    ...
    `SaveState` v3(`questStage`). **한 가지 알려진 제약** — 순서를
    벗어나(예: 미니보스보다 구출을 먼저) 끝내면 `MarkCaptiveFreed()`가
    "아직 그 단계가 아니다"로 무시하고 재시도가 없어 그 진행이
    영영 인정 안 된다(웹판도 사실상 선형이라 진행 순서를 강제하는
    구조라 크게 벗어나진 않지만, 방 종류 마지막 슬라이스 이후로는
    Room3/4를 순서대로 지나야만 하는 구조라 실제로는 못 벗어난다 —
    이론상의 결함으로 남겨 둠).
...
- **DUNGEON PLAN 챕터 순 심화 — 이번 세션은 퀘스트(36장)·미니맵(27장)·
  타격감 일부(38장) 세 조각에서 멈췄다(2026-09-12, 아홉 번째 세션,
  위 "완료 단계" 참고).** 사용자가 "다해줘"로 계속하라고 했지만 세
  조각도 아직 사람이 GUI로 하나도 확인 안 한 채라 다음 세션이 이어갈
  때 먼저 훑을 것. 웹판 PLAN.md 챕터 순으로 이 던전 규모(방 넷짜리
  선형 통로)에 아직 안 맞은 다음 후보들:
  - **35장 랜덤 이벤트** — Monster Ambush·Secret Area는 아직 이
    슬라이스에 대응물이 없다(Merchant·Treasure·NPC Rescue·Elite
    Monster·Shrine·Mini Boss는 이미 고정 콘텐츠로 다 있음). 다만 이
    씬은 "테스트 씬은 전부 고정 좌표" 원칙이라 room/layout 절차 생성은
    여전히 안 하고, LuckyCairn(GO)처럼 위치는 고정하고 결과만 확률
    룰렛으로 굴리는 절충이 필요.
  - **37장 사운드** — 이 프로젝트엔 아직 오디오 에셋 파이프라인 자체가
    없다(원작 에셋 반입 금지 원칙상 CC0 SFX를 새로 구해야 함) — 다른
    슬라이스보다 선행 조사가 더 필요해 보류.
  - **26장 HUD 개선 나머지** — 지금 HUD는 텍스트 한 줄 뭉치뿐, 체력
    바(게이지) 등 시각적 표시는 아직 없음.
  - **41장 최종 UX 목표** — 다른 장이 갖춰진 뒤 마지막에 재점검.
  세션 시작 시 이 문서와 `saga-dungeon/PLAN.md`를 다시 훑어 사용자가
  고르는 대로 이어갈 것.
