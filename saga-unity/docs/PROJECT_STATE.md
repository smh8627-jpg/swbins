...
마지막 갱신: 2026-09-17 (PLAN 101-2 A·B **다섯 판 전부 이식 완료** + **105 Q-U5 DoF 토글** + **101-3 C 전부(hitstop/shake/flash/popup/타격 VFX) GO·DUNGEON·STORY 완료** + **101-3 F 죽음·G 성장 연출·G 장비 가시화(무기 소켓) DUNGEON 구현 완료** + 다섯 게임 컴파일·씬 재생성·헤드리스 3연속 검증 완료).
...
| DUNGEON | `TestDungeon` | 완료 — 첫 방→무리·엘리트/보스·방 종류(우물·상자·성소·행상)·회피·강공격·필드(방 2+복도)·동행 | 마을 넷·층 진행·매복·구출·수수께끼·은닉 창고·빌드(회전베기)·도감·보석/영웅 상태 | Player·잡졸(황건적)·미니보스/두목·Environment·Building | 전부 붙음(SFX 실클립 통일). **목표판/세션카드(101-2 A·B, 2026-09-17 검증 완료)**. **hitstop·타격 VFX·죽음 표식·성장 연출·무기 소켓(101-3 C·F·G, 2026-09-17)** |
...
- **101-3 C 전부(hitstop/shake/flash/popup/타격 VFX) GO·DUNGEON·STORY 완료** — hitstop은 Animator.speed=0(가해자+피해자, 웹판 원문 크리티컬 전역 슬로모와 별개 공존), VFX는 `HitSpark`(PC도 Shuriken 재사용, VFX Graph는 에디터 그래프라 안 씀), GO는 초당 판정(`DuelRules.Step`)이라 hitstop만(`BanditEncounter`/`RareWolfEncounter`).
- **101-3 F 죽음(`LootMarker`)·G 성장 연출(`CameraRig.PlayLevelUpCut`) DUNGEON 완료** — 유품 마커는 회수 반경 2m·추가 보상 없음·12초 소멸. 성장 연출은 Cinemachine 패키지가 없어 수동 줌 코루틴(1.2s)으로 대신, 아무 키나 누르면 스킵.
- **101-3 G 장비 가시화(무기 소켓) DUNGEON 완료(2026-09-17)** — `CharacterVisual.FindOrCreateWeaponSocket()`(Humanoid `HumanBodyBones.RightHand`, 아니면 시각 루트 밑 고정 오프셋 폴백. **`animator.isHuman`으로 먼저 거를 것** — Animator가 있어도 Avatar 미설정/비휴머노이드면 `GetBoneTransform`이 예외를 던진다) + `WeaponVisual`(자루+칼날 프리미티브를 코드로 짓는다, 무기 메시 자산 없음). `ItemData.Grade`(0~2, wp_start·wp_axe=0/wp_saber=1/wp_glaive·wp_greatblade=2)에 따라 칼날 길이·이미시브 림(0=무광·1=옅은 청록·2=강한 금색)이 갈린다. `HeroState.EquipmentChanged`(신규 이벤트, `EquipIfBetter()`가 실제로 바뀔 때만 동기 호출)로 갱신. 남은 101-3은 **G 지형 반응(데칼)** 하나뿐 — 시작 전 사용자와 방향(URP Decal Renderer Feature 배선 범위) 확인 필요.
...
3. ~~PLAN 101-3 C·F·G(성장 연출·무기 소켓)~~ — hitstop·shake·flash·popup·타격 VFX(GO·DUNGEON·STORY)·유품 마커·성장 연출·무기 소켓(DUNGEON) 전부 완료(2026-09-17). 남은 101-3은 **G 지형 반응(데칼, URP Decal Renderer Feature+전용 셰이더 새로 배선해야 함)** 하나뿐 — 사용자와 방향 먼저 확인.
4. **실기 GUI 확인 몰아서** — 아래 "실기 확인 대기" 전부(다섯 SettingsPanel + 다섯 판 목표판/세션카드 + 타격 VFX + 무기 소켓 포함). 사용자가 직접 하거나 명시 요청 시(폴더 CLAUDE.md).
...
- 없음(컴파일·헤드리스 기준, 2026-09-17 다섯 게임 전부 재확인 + 무기 소켓 추가 뒤 DUNGEON 재확인).
...
- **`Animator.GetBoneTransform()`은 `isHuman`으로 먼저 거를 것** — Animator 컴포넌트가 있어도 Avatar가 비어 있거나 Humanoid가 아니면 `InvalidOperationException: Avatar is null`을 던진다(2026-09-17 `WeaponVisual` 작업 중 이 PC의 Maria 인스턴스가 실제로 그 상태라 겪음 — `CharacterVisual.FindOrCreateWeaponSocket()`이 `animator.isHuman` 가드로 고쳐 둠, 앞으로 Humanoid 본을 참조하는 새 코드는 전부 같은 가드 필요).
...
- **`animator?.SetTrigger(...)`(null-조건 연산자) 쓰지 말 것** — 애셋 폴백으로 "직렬화 때 한 번도 안 채워진" `UnityEngine.Object`는 진짜 C# null이 아니라 `?.`가 못 거르고 `UnassignedReferenceException`을 던진다. `if (animator != null)`로 명시적으로 막을 것(2026-09-17 STORY `PlayAttackAnim()`에서 실제로 겪음).

## 테스트 상태 (2026-09-17 기준, 전부 배치 모드, 이 PC Unity 6000.3.24f1로 실제 실행)

| 검증 | 결과 |
|---|---|
| `-batchmode -nographics -quit` 컴파일 | exit 0, 오류 0(라이선스 토큰 경고만, `WeaponVisual` 추가 뒤 재확인) |
...
| `PlaytestDungeonHeadless`(hitstop·타격 VFX·유품 마커·`CheckLevelUpCut`·`CheckWeaponVisual`(무기 등급 갱신 시 칼날 크기 변화) 실제 검증 포함) | 3연속 OK(`WeaponVisual` 추가 뒤 재검증 — 첫 시도는 `Animator.isHuman` 가드 누락으로 런타임 예외 1건, 수정 후 3연속 통과). 하위 슬라이스(`FloorProgression`·`FieldAmbush`·`Shortcut`·`Town2`·`Towns34`)는 DoF 반영 시점에 1회씩만 재검증 — 이후 추가분(HitSpark/LootMarker/PlayLevelUpCut/WeaponVisual)은 아직 재확인 안 함(기존 경로만 공유해 위험 낮다고 판단) |
| `PlaytestForestHeadless` | 3연속 OK(DoF 반영 뒤 재검증). 하위 슬라이스도 1회씩 재검증 — 전부 OK, 회귀 없음 |
| `PlaytestOverworldMap`(GO) | 1회 재검증 — OK, 회귀 없음 |
| `PlaytestStorySlice`(전직·언어 전환·save/load, hit feedback·타격 VFX 검증 포함) | 3연속 OK(이 씬 player Animator는 null이라 hitstop 자체 검증은 스킵, shake·VFX는 확인됨) |
...
- GO: 조우·전투·등용 손맛, 상점·퀘스트 대사 3단계, 은닉 보물·산신당·돌탑·유물, 동물 Group 발동 장면, 채집, 드로우콜 Before/After(Stats 창), **목표판 3줄 화면 배치(대화창과 안 겹치는지)·세션카드 실제 등장(무입력 5분 체감)**, **hitstop 체감(도적·늑대 둘 다 — 101-3 C)**
- DUNGEON: 카메라 각도·공격 버튼 vs CameraRig 드래그 겹침, 아홉 슬라이스 전부, 마을 넷·층 진행·매복·구출·수수께끼·회전베기, 사실적 Player/Enemy/Boss 톤, **목표판/세션카드**, **hitstop 체감(70/120ms)**, **타격 VFX·유품 마커 색·크기 체감**, **레벨업 줌 펀치인 체감(1.2s)**, **무기 소켓 실제 손 위치·각도 체감(리깅된 "쥔" 포즈가 아니라 손 본 자리에 얹은 근사치라 어색할 수 있음, 등급별 칼날 크기·이미시브 색 체감 — 101-3 G)**
