...
- **DUNGEON 오픈월드 확장 — 열 번째 세션에서 첫 조각(Room5·Room6, 층2
  진입)을 끝냈다(위 "완료 단계" 맨 위 참고).** 방향은 "물리적 확장
  계속"으로 확정됐으니 다음 조각도 같은 결로 이어가면 된다 — 후보:
  (a) Room7·Room8로 층2를 더 채우기(웹판 makeRoom 종류 재활용, 예:
  층2 정예/미니보스), (b) Room6(층2 두목)을 클리어하면 "층3"으로
  올라가는 사다리꼴 패턴을 한 번 더 반복해 "층이 정말 깊어진다"는
  감각을 굳히기, (c) 사람이 먼저 Room5·Room6을 실기로 확인한 뒤
  피드백으로 다음을 정하기 — **사용자가 "실기는 마지막에, 다른 작업이
  우선"으로 정해 뒀으니 실기 확인을 기다리지 말고 (a)나 (b)로 바로
  이어가도 된다.** 사용자가 "새로운 세션에서 이어 하자"로 끊지 않는
  한 이번 세션 안에서 계속 진행 가능.
