...
마지막 갱신: 2026-09-20 (이어서 **PLAN 101-2 5-6 REALM 지형·전술** — 지형 Plain/River 둘뿐이라 평야=기병 돌격(무력80+,×1.25)·강=화공(지력60+,×0.85)으로 좁힘, 진형 수동 선택은 시스템 없어 스코프 밖. `RealmWar.Fight` 배율 두 자리+`ResolveTactic()`+`RealmCommandUi` 토글. **부수 수정**: `BuildTestCityScene.Build()`가 언어(PlayerPrefs) 안 고정해 직전 실패 헤드리스의 "en" 잔존이 다음 씬 재빌드를 깨는 걸 겪어 "ko" 고정 추가. 컴파일 0 오류, 헤드리스 3연속 OK). DUNGEON 전부 완료(5.7만 웹 선행 뒤). FOREST(5.1~5.5,5.8①) 완료 — 남은 건 5.6·5.7·5.8②③. REALM(5-1,5-6) 완료 — 남은 건 5-2·5-8.
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 55, 성 58**(51장 35차, 세 사슬 닫힘) · **5-1** 특성·야망 · **5-6(2026-09-20)** 지형 전술(`RealmOfficerTraits`·`RealmWarState.ResolveTactic`) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(A·B — "월간 요약 카드")**. 101-3 해당 없음 |
...
1. **PLAN 101-2 이어서** — GO④⑦③·DUNGEON5.1~5.5·FOREST5.1~5.5,5.8①·REALM5-1,5-6 완료. 다음: GO①②⑥⑧(사용자 결정 대기, ⑤는 모바일 빌드 뒤)·DUNGEON5.7(웹 선행 뒤)·FOREST5.6/5.7/5.8②③·REALM5-2/5-8. STORY 5-2~5-4·5-8은 "적이 반격 안 함" 등 선행 시스템이 없어 재검토 필요.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-20, 5-6 지형 전술 뒤 — 신규 `CheckTactic()` 포함, 씬 재빌드로 토글 버튼 배선 확인) |
...
- REALM: 월드맵, 적국 사슬 체감, 패널 여덟 조작, 목표판/세션카드, 공격·계략 고르기, 특성 배지·야망 진행도(5-1), **전술 토글·지형 힌트·기병 돌격/화공 체감(5-6, 2026-09-20 신규 — 첫 확인)**
