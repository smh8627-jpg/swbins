...
마지막 갱신: 2026-09-18 (이어서 **REALM 51장 18차 확장**(창오→울림·건녕→월수, 아래 절 참고) — 17차(남해→창오·주제→건녕) + 계략 고르기 UI 완료도 같은 세션. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 31, 성 34**(51장 18차 — 16차가 국경 성 3곳에 연 둘째 목표(천수·남해·주제)에서, 17·18차가 교주·남중 두 사슬을 각각 두 단계 더 뻗었다: 남해→창오→울림, 주제→건녕→월수) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
## REALM 51장 16~18차 — 국경 확장 (2026-09-18, 16차는 Q-U2 결정)
...
17·18차: 그 두 사슬을 각각 두 단계씩 더 뻗었다 — 남해→창오→울림(교주), 주제→건녕→월수(남중). 이때부턴 16차의 "형제 가지"가 아니라 정상적인 한 단계 더 깊은 자식이라 train은 옛 규칙(부모 train+15)을 그대로 씀. 남해의 다른 이웃(합포)·건녕의 다른 이웃(장가·운남)은 다음 확장 후보로 남김. 월수는 원작 LINKS상 더 이상 이웃이 없어(잎사귀) 남중 사슬은 여기서 끝.
- **실제로 걸린 함정(17차)**: `RealmEnemyCity.cs`에만 새 성을 추가하고 `RealmCityData.cs`(플레이 가능 성의 agri/comm/pop/좌표 정의)를 빠뜨려 첫 검증이 실패했다 — `RealmCityState.AbsorbCity()`가 그 정의가 없으면 **에러 없이 조용히 return**한다. 함락 자체는 `Ok=true, Won=true`로 성공해 버려서, 헤드리스 검증이 `ActiveCityIds.Contains()`로 편입 여부를 따로 확인하는 덕에 잡혔다. **새 REALM 성은 항상 두 파일 다 같이 고칠 것**(18차부터는 이 규칙을 지켜 처음부터 안 걸림).

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널·17·18차 신규 성 넷도 포함). 사용자 몫.
2. **PLAN 104-1 ⑤·102-4** — `Assets/Art/*_candidates` 정리, 105 Q1 결정 대기.
3. **REALM 교주·남중 더 뻗기(19차 후보)** — 남해→합포, 울림→교지, 건녕→장가/운남(허브라 갈래 여럿). 사용자와 방향 상의(또는 그대로 "계속 확장" 기조 유지, 지금까지처럼 물으면 이어감).
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI·51장 17·18차 확장 모두 2026-09-18에 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-18, 51장 16~18차 확장 + 계략 고르기 UI 뒤 — 장안 다중 목표(2개) 공격·계략 고르기 패널(각 2/4버튼)·잘못된 목표 거절·천수/남해/주제/창오/건녕/울림/월수 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안 등 목표 둘인 성) 실제 조작감(51장 16~18차 + 계략 고르기 UI, 2026-09-18 신규 — 이게 첫 실기 확인)**
