...
- **사용자가 "1,2 순서대로 진행"으로 확정 (2026-09-12, 열두 번째 세션) —
  (1) FOREST가 문제없다는 실기 확인 뒤 단순화해 둔 부분 채우기 → (2)
  DUNGEON 진짜 오픈월드 확장.** (1)의 두 조각(바이옴 지형 다양성, 창조물
  종 늘리기 — 포자괴물·안개유령)을 이 세션이 끝냈다(위 "완료 단계" 참고).
  **다음 세션이 볼 것**:
  - 사람이 이번 두 슬라이스(바이옴·신규 종 둘)를 실기로 확인한 피드백이
    있으면 그것부터.
  - 없으면 (1)의 나머지 후보 중 하나를 마저 채운다 — **가구 자유
    배치**(지금은 고정 자리 여섯, `ForestFurnitureAnchor.cs` — "다가가서
    상호작용" 대신 실제 좌표에 놓으려면 새 입력 동사가 필요해 파급이 큼)
    또는 **벽지/장판**(`ForestHomeData.cs`에 카테고리 추가 필요, godot도
    같은 이유로 범위 밖에 둬 왔다) 또는 **창조물 종을 더 늘리기**(여섯
    종 → saga-godot FOREST 5절 "몬스터·퓨전 자유" 결정 범위 안에서
    계속 자유롭게, 예: 바위 지대·꽃밭도 두 종씩으로 채우기). 어느 걸
    먼저 할지는 방향 결정이라 세션이 임의로 고르지 않는다.
  - (1)을 충분히 채웠다고 판단되면 (2) DUNGEON 오픈월드 확장으로
    넘어간다(아래 항목 — saga-dungeon 웹판의 실제 방 배치/복도 연결
    알고리즘부터 훑어 범위를 정할 것).
