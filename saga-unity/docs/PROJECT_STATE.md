...
마지막 갱신: 2026-09-17 (REALM 51장 15차 + Localization 잔여 감사(완료 확인) + PLAN 101-2 A·B DUNGEON·FOREST·STORY 이식(GO 포함 넷 완료, REALM만 남음) — 이 PC 에 Unity 없어 컴파일 미검증).
...
| FOREST | `TestVillageForest` | 완료(이동 전용 컨트롤러) — 마을·집·주민 | 벽지/장판·가구 자유 배치(1m 격자)·생물(Flee/Group)·과일나무·채집·좌판·밀어내기 전투 | Environment 완료 | 전부 붙음(데이터 콘텐츠 en 번역도 2026-09-17 감사로 완료 확인). **목표판/세션카드(101-2 A·B, 뼈대, 이번 세션 이식)** |
| STORY | `TestField` | 완료 — 2.5D 횡스크롤(Z 고정)·잡졸 10·두목·사명 2·볼트·로프 | 척후병 NPC·사건·관계·선택(51장 완결)·전직(Lv.10, 무사/궁수/협객/방사) | 척후병 실제 모델 | 전부 붙음. **목표판/세션카드(101-2 A·B, 뼈대, 이번 세션 이식)** |
...
- **PLAN 101-2 A·B — DUNGEON·FOREST·STORY 세 판 이식 완료(GO 포함 넷, REALM만 남음)** — 셋 다 GO `GoSessionTracker.cs` 패턴 그대로, "지금"/"이번 세션" 지표만 판마다 있는 걸로 교체: DUNGEON은 `DungeonEnemy.FindNearest()`(적까지 거리)+금, FOREST는 가장 가까운 `ForestFruitTree`(무제한 채집이라 "다 찾음" 개념 없음)+`ForestState.FruitCount`(이 판엔 금 경제가 없어 과일로 대체), STORY는 `StoryEnemy.All`(!IsDead)+`StoryQuestState.Kills`(이 판도 금·과일 없음). 각 `UI/*SessionTracker.cs` 신규 + `BuildTestXxxScene.cs`의 `BuildGoalBoardUi()`(`BuildSettingsUi()` 뒤) + `Playtest*.cs`의 `CheckGoalBoardAndSessionCard()`(GO 버전 복사, 클래스명만 교체) — 단 STORY의 `PlaytestStorySlice.cs`는 프레임카운트식이 아니라 Phase 상태머신이라 새 Phase 없이 `Phase.Init` 안에서 bool 반환으로 맞춰 끼움(이 파일 기존 `Check*()` 관례). SagaCore(GoalBoard/SessionCard/IGoalSource)는 손 안 댐 — GO 이식 때 짠 공용 컴포넌트 그대로 재사용. 새 `.cs.meta`는 GO 넷도 없던 전례를 따라 안 만듦(Unity가 다음에 열릴 때 자동 생성). REALM만 남았다("일과" 개념이 경영형과 안 맞을 수 있어 이식 전에 검토 필요).
- **테스트 상태 표(아래)는 전부 이번 세션 편집 이전 결과다.** 이번 세션에 고친 파일(REALM 12~15차 3개 + GoalBoard DUNGEON/FOREST/STORY 6개 + 이전 세션 미검증분) 전부 재검증 전.

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **컴파일·재검증(최우선)** — 누적 미검증분(104-1 9개 + 101-2 GO·DUNGEON·FOREST·STORY 네 판 이식분 + REALM 51장 10~15차 10개) 전부. Unity 에디터 있는 세션에서: 배치 컴파일 → `BuildTestXxxScene`(GO/DUNGEON/FOREST/STORY) + `BuildTestCityScene`(REALM) 재생성 → `PlaytestHeadless`·`PlaytestDungeonHeadless`·`PlaytestForestHeadless`·`PlaytestStorySlice`·`PlaytestRealmSlice` 3연속. 씬을 안 다시 지으면 [SerializeField] 승격·GoalBoard 배선(네 판 전부)·건업·회계·운중·상군·삭방·오원 사슬 전부 실제로 검증되는 게 없다.
2. **실기 GUI 확인 몰아서** — 아래 "실기 확인 대기" 전부(다섯 SettingsPanel + GO·DUNGEON·FOREST·STORY 목표판/세션카드 포함). 사용자가 직접 하거나 명시 요청 시(폴더 CLAUDE.md).
3. **PLAN 101-2 A·B 마지막 한 판(REALM)** — GO·DUNGEON·FOREST·STORY 넷 이식 완료(이번 세션). REALM 은 "일과" 개념이 경영형(캐릭터 없음, 명령·계략 중심)과 안 맞을 수 있어 이식 전에 먼저 검토 필요 — "지금"·"이번 세션"에 뭘 넣을지부터 사용자와 상의하거나 신중히 판단할 것(감으로 먼저 짜지 말 것).
