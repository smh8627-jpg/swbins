...
1. **PLAN 104-1 ⑤·102-4 마무리** — Shaders/Character·Environment/PBR 승격, `CharactersVroid/` 삭제, `Props/`(fence·fenceGate) PBR 교체까지 완료. 남은 건: `Rocks/`·`Vegetation/`(`procgen.py`, 103장 에셋 생성 작업) — **2026-09-21 Blender 설치 완료(winget), 착수 가능**(procgen.py에 tree kind 추가부터) · `Props/`의 lantern·stall-red(재질 혼재라 보류, 실기 확인 후 재판단) · `Characters/` Kenney는 **아직 못 뺀다**(GO 플레이어·GO/FOREST/STORY 주민·STORY 잡졸이 실사용 중).
