...
- **지도 크기는 9×11에서 일단 멈췄다** — 남쪽 4줄+동쪽 2칸을 늘리고
  ...
  같이 볼 것.
- **GO 콘텐츠 다양화 다음 후보.** PLAN.md 24~27장 이벤트 종류 중
  "지역/시간/랜덤 이벤트"는 아직 하나도 없다(몬스터 출현·보물 발견·NPC
  이벤트·희귀 몬스터는 이제 다 있음) — 예: 특정 시간대에만 나오는
  것·확률로 갈리는 조우 등. 그 외엔 사건/퀘스트/상점을 사전 구조로
