...
마지막 갱신: 2026-09-24 (서른아홉 세션째 — **PLAN 107장 GO 원신 기준 ①②③④ 완료**: 들판 전투 · 이동(절벽 고원·깊은 강·점프·등반·활공·수영) · 지역 지도(지역 7·역참 5·옛 망루·봉우리·M 지도·세이브 v14) · 보물 상자 16(등급 4 × 잠금 3: 없음·무리 전멸·원소 석등). 헤드리스 통과, 실기 확인은 전. 경위는 HISTORY 날짜 grep).
...
|---|---|---|---|---|---|
| GO | `TestVillage` | 완료 — 도적의 습격(이동·촌장·상인·나그네·조우·전투·등용·EXP·장비·루트·저장 v13) | 동물 Group·나그네·은닉 보물·산신당·행운 돌탑·동굴 유물·채집 · 101-2 ④⑦③: 일과판·승급 3택·75초 토벌 · ①⑥⑧: 봉수대·인연·패배 비용·회수 · ②: 사당 시련(파도 3·인장 조각) · **107 ①②③ (2026-09-24)** 들판 전투 + 이동 + 지역 지도(역참·망루·M 지도, 세이브 v14) | Player·주요 Enemy·Environment·Building 전부 GLB/PBR, Props는 fence·fenceGate만(lantern·stall 보류), Rocks/Vegetation은 procgen 트라이플레이너(102-4) | 전부 붙음. 목표판/세션카드(A·B). 101-3 C·F·G 전부 완료 |
...
0. **PLAN 107장 GO 원신 기준이 최우선**(사용자 2026-09-24 "지금 바로"). ①②③④ 완료 → 다음 **⑤ 원소 쓰는 적**(원소 방패: 같은 원소 면역·물리 ×0.4·상성 ×2.5, 깨지면 비틀거림 — 107 표) → ⑥ 동료 모델. ③ 남은 것: 걸어 오르는 경사·고개, 지역마다 바이옴. DUNGEON 106 순서 5 는 GO 이동 상태 판별 복사.
0-1. **107 ①② 남은 것**: 새 글자 `field.*`·`move.*`·`chest.*`·`map.*` 가 `GoLocalization.T(키, 한국어)` 폴백 — ko/en 표(다섯 벌 md5)에 넣기. 동료 교체는 몸이 안 바뀌고 원소 빛깔만(⑥). 넘어오르기 전용 클립 없음(0.45s 코드 이동). 다른 PC 는 Maria 이동 클립 5개를 README 레시피로 받고 `Saga/Build Maria Traversal` 한 번.
...
| `PlaytestStorySlice` | **3연속 OK(2026-09-23, 옷 빛깔 뒤)** — `CheckOutfitTint`·`CheckUpperTiersAndPins`·`CheckPromotionAndSchools`·`CheckButtonWiring`(진짜 onClick)·`CheckJobSkills`·무예 세이브 왕복·옛 형식 로드(세부는 HISTORY 09-23) |
...
| GO `PlaytestHeadless` | **3연속 OK(2026-09-24, 107 ④ 뒤)** — `PlaytestGoTreasure`(상자 16·다가가 열기·높이·무리 한꺼번에 전멸·석등 원소/20초/진짜 스킬·지도 개수·세이브 왕복) + `PlaytestGoWorldMap`(역참·망루 실제 등반·지도·세이브 v14/v13) + `PlaytestGoTraversal`(점프·등반·활공·수영 측정값) + `PlaytestGoFieldCombat`(반응·3타·스킬/폭발·회피·교체·전멸) |
...
- GO: **보물 상자(107 ④) — 상자 크기(2.2m)·등급 띠/빛이 멀리서 구분되는지, 사슬 X 가 읽히는지, 뚜껑 젖힘, 잠금 안내 문구, 석등 구슬 빛깔이 원소로 읽히는지·20초가 빠듯한지, 무리 상자 "한꺼번에" 체감, 높은 곳 상자(망루·봉우리·고원)가 보이는지, 보상량(경험치 5~60·돈 10~100냥)** · **지역 지도(107 ③) — 역참 돌기둥·구슬 빛, 옛 망루가 마을에서 보이는지·오르는 맛, 봉우리 모양, M 지도 가독성(어둠·◆·화살표 방향), 지역 이름 표시 빈도** · **이동(107 ②) — 절벽 고원 모양·높이, 등반 속도/클립(Climbing Up A Wall 이 벽에 붙어 보이는지)·넘어오르기 0.45s 가 튀지 않는지, 활공 날개·자세, 수영 높이(머리만 물 위)·물 위 대기, 다리 7m 폭 걷기, 스태미나 소모 체감, 카메라가 절벽에 끼지 않는지** · **들판 전투(107 ①) — 3타 간격·예고 0.6s 가 피할 만한지·적 체력(산적 320·해골 220)·HUD(체력·기력·스태미나·명단·버튼) 자리·교체 빛깔·산적/해골 크기**, 조우·전투·등용 손맛, 상점·퀘스트 대사, 은닉 보물·산신당·돌탑·유물, 채집, 목표판/세션카드, hitstop, 유품 마커·무기 소켓·지형 데칼, 일과판·승급 3택, 75초 토벌, 봉수대·인연·짐 드롭/회수, 사당 시련, 울타리 목재 톤, 나무·바위 트라이플레이너 톤, 마을집 실루엣, 카메라 벽 pull-in, **폰에서 설정·승급 3택·저장 버튼이 눌리는지(2026-09-23 고침)**
