...
마지막 갱신: 2026-09-18 (이어서 **REALM 계략(Plot) 고르기 UI 완료** — 51장 16차 확장이 남긴 "알려진 틈"(목표 둘인 성에서 계략이 카탈로그 첫째에만 걸리던 문제) 해소. `RealmCommandUi.RefreshPlotPanel()`이 목표가 둘인 성(장안·장사·강주)에서 "계략×목표" 조합(2종×2곳=4버튼)을 내고, `ChoosePlot(key, enemyId)`가 그 선택을 `RealmWarState.Plot()`에 넘긴다 — `Attack()`/`ExecuteAttack()`과 같은 결. 헤드리스 검증(`PlaytestRealmSlice.CheckMultiTargetPlot()` 신설, 실제 계략은 안 걸고 패널·버튼 개수·잘못된 목표 거절만 확인) 3연속 OK. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션). STORY로 F 죽음·G 지형 반응·G 성장 연출 이식 + G 장비 가시화를 "직업별 무기 소켓"으로 재해석해 구현. **REALM Q-U2 결정 — 51장 사슬 16차 확장**: "성 하나당 목표 하나" 제약을 풀고 국경 성 3곳(장안·장사·강주)에 둘째 목표를 열었다(`RealmEnemyCity.TargetsFrom()`, `RealmCommandUi` 공격 고르기 패널).
...
- ~~**알려진 틈**: `Plot()`(계략)은 아직 고르기 UI가 없다~~ — **2026-09-18 해소**. 아래 "다음 작업" 참고.

## 다음 작업 (우선순위, 상세는 PLAN 해당 장 · 경위는 HISTORY 날짜 grep)

1. **실기 GUI 확인 몰아서** — "실기 확인 대기" 전부(아래 목록, STORY·REALM 신규 항목 포함, REALM엔 계략 고르기 패널도 새로 추가됨). 사용자 몫.
2. **PLAN 104-1 ⑤·102-4** — `Assets/Art/*_candidates` 정리, 105 Q1 결정 대기.
3. **REALM 교주·남중 더 뻗기** — 남해→창오/합포, 주제→건녕. 사용자와 방향 상의(또는 그대로 "계속 확장" 기조 유지).
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI는 2026-09-18에 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-18, 51장 16차 확장 + 계략 고르기 UI 뒤 — 장안 다중 목표(2개) 공격·계략 고르기 패널(각 2/4버튼)·잘못된 목표 거절·천수/남해/주제 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안 등 목표 둘인 성) 실제 조작감(51장 16차 + 계략 고르기 UI, 2026-09-18 신규 — 이게 첫 실기 확인)**
