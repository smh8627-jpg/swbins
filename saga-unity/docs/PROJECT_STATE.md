...
- **DUNGEON 다음 슬라이스 후보** (`docs/VERTICAL_SLICE_DUNGEON.md`
  "다음 슬라이스 후보" 절 참고) — 2026-09-12 일곱 세션에 걸쳐 열네
  ...
  **회피 애니메이션·이펙트(구르기 회전·잔상·무적 틴트, 위 "완료 단계"
  참고)**.
  ...
  후보: **방 셸 GLB**(room-small.glb — 위 완료 단계에 적어 둔 대로 방
  치수를 GLB 원본(12×12)에 맞추는 재설계가 먼저 필요, saga-godot
  `test_room.gd`가 이미 그 길로 감) 정도가 눈에 띄는 큰 조각으로 남고,
  그 외엔 `VERTICAL_SLICE_DUNGEON.md`의 원래 "다음 슬라이스 후보" 목록을
  거의 다 끝냈다 — saga-dungeon 웹판 PLAN.md 챕터 순서(오픈월드 확장
  등 더 큰 콘텐츠 확장)를 참고해 사용자가 고르는 대로.
