...
마지막 갱신: 2026-09-18 (이어서 **REALM 51장 20차 확장**(교지→구진·남해 둘째 목표 합포, 아래 절 참고) — 16~19차(국경 성 확장+계략 고르기 UI+두 사슬 계속 뻗기)도 같은 세션. **PLAN 101-3 C·F·G — 다섯 판 전부 적용 완료**(이전 세션).
...
| REALM | `TestCity` | 완료(경영형, 캐릭터 없음) — 명령·계략(유언비어·화계)·문답 36·서고·월드맵·전투·함락 편입 | **적국 35, 성 38**(51장 20차 — 교주 사슬 남해→{창오→울림→교지→구진, 합포}, 남중 사슬 주제→건녕→{월수,장가}. 남해·건녕 둘 다 형제 가지 규칙으로 목표 둘씩) | 도시 Environment/Building | 전부 붙음. **목표판/세션카드(101-2 A·B — "월간 요약 카드"로 변형)**. 101-3 해당 없음(캐릭터·실시간 전투 없음) |
...
## REALM 51장 16~20차 — 국경 확장 (2026-09-18, 16차는 Q-U2 결정)
...
17~20차: 두 사슬을 계속 뻗었다 — 교주: 남해→{창오→울림→교지→구진, 합포}, 남중: 주제→건녕→{월수,장가}. 대부분은 정상적인 한 단계 더 깊은 자식(train=부모+15)인데, **19차의 장가·20차의 합포는 각각 건녕·남해의 둘째 목표**(형제 가지) — 16차 규칙을 원래 세 국경 성 밖으로 두 번 더 확장한 사례다. `TargetsFrom`/고르기 패널이 이미 목표 개수와 무관하게 동작해 코드는 안 고치고 데이터만 늘렸다(그래서 새 목표를 추가할 때마다 그 도시를 출진지로 쓰는 기존 `AttackChainStep`은 `enemyId`를 명시로 바꿔야 한다 — 안 그러면 어느 목표인지 불명확). 월수는 잎사귀(더 이상 이웃 없음)라 그 갈래는 끝. 남은 후보: 구진→일남, 건녕→운남.
...
3. **REALM 교주·남중 더 뻗기(21차 후보)** — 구진→일남, 건녕→운남. 사용자와 방향 상의(또는 그대로 "계속 확장" 기조 유지, 지금까지처럼 물으면 이어감).
4. **PLAN 105 열린 질문(Q1·Q3′·Q4·Q-U3·Q-U4)** — 전부 사용자 결정 대기(Q-U2는 2026-09-18 해결·삭제됨).

REALM 계략(Plot) 고르기 UI·51장 17~20차 확장 모두 2026-09-18에 완료됐다 — 다음 세션이 새로 이어받을 잔여 작업 없음.
...
| `PlaytestRealmSlice` | **3연속 OK**(2026-09-18, 51장 16~20차 확장 + 계략 고르기 UI 뒤 — 장안·건녕·남해 다중 목표 공격·계략 고르기 패널·잘못된 목표 거절·10개 신규 성 함락 전부 새 검증 포함) |
...
- REALM: 월드맵, 적국 사슬 진행 체감, 패널 여덟 실제 조작, 목표판/세션카드, **공격·계략 고르기 패널(장안·건녕·남해 등 목표 둘인 성) 실제 조작감(51장 16~20차 + 계략 고르기 UI, 2026-09-18 신규 — 이게 첫 실기 확인)**
