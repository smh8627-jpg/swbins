...
{
    /// <summary>
    /// PLAN.md 106-3 — 컷 전용 CinemachineCamera 에 붙는 "달리" 한 개. 두 점(from→to) 사이를
    /// 부드럽게 미끄러지며 두 시선점(lookFrom→lookTo)을 본다. `CutsceneDollyTrack` 이 클립 진행도로
    /// `Apply(t)` 를 부르고, 상자 컷처럼 자리가 매번 다른 컷은 `DungeonCutscenes` 가 `Set` 으로 다시 잡는다.
