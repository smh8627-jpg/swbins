...
namespace Saga.Dungeon.UI
{
    ...
    public class DungeonSaveButton : MonoBehaviour
    {
        public void Save()
        {
            bool ok = SaveState.Save();
            DialogueLabel.Instance?.Show(ok ? DungeonLocalization.T("command.save_ok", "저장했다.")
                : DungeonLocalization.T("command.save_fail_player", "저장 실패 — 플레이어를 못 찾았다."), 3f);
        }
    }
}
