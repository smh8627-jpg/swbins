...
{
    /// <summary>
    /// `Saga.Story.UI.LocalizedButtonLabel`·`Saga.Dungeon.UI.LocalizedButtonLabel`과
    /// 같은 결(2026-09-15) — 씬 빌드 시점 언어로 굳는 저장 버튼 글자가
    /// 플레이 중 언어 전환에 반응하게 한다. 매 프레임 언어 문자열 하나만
    /// 비교하는 가벼운 폴링.
    ///
    /// **함정** — 참조 필드를 일반 private으로 두고 에디터 빌드 스크립트가
    /// `Init()`으로 한 번만 채우면, 씬 저장·재로드 후 Play 모드에서 전부
    /// null로 돌아온다(DUNGEON/STORY 쪽에서 이미 밟았다). `key`/`fallback`은
    /// `[SerializeField]`로 승격, `_text`는 Awake()에서 다시 찾는다.
    /// </summary>
    public class LocalizedButtonLabel : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            string lang = GoLocalization.CurrentLanguage;
            if (lang == _lastLang) return;
            _lastLang = lang;
            _text.text = GoLocalization.T(key, fallback);
        }
    }
}
