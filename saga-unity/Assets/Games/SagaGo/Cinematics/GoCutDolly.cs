...
{
    /// <summary>
    /// PLAN.md 106-9(DUNGEON 106-3 판별 복사) — 컷 전용 CinemachineCamera 에 붙는 "달리" 한 개. 두 점(from→to) 사이를
    /// 부드럽게 미끄러지며 두 시선점(lookFrom→lookTo)을 본다. `GoCutDollyTrack` 이 클립 진행도로
    /// `Apply(t)` 를 부르고, 두목 등장처럼 자리가 매번 다른 컷은 `GoCutscenes` 가 `Set` 으로 다시 잡는다.
    /// Cinemachine 의 스플라인 달리 대신 이걸 쓰는 건 컷마다 직선 한 줄이면 충분해서다.
    /// </summary>
    public class GoCutDolly : MonoBehaviour
    {
