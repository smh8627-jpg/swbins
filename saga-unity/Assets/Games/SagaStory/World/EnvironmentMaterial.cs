...
{
    /// <summary>
    /// 44장 "Environment" 교체 — Saga.Dungeon.World.EnvironmentMaterial.cs와
    /// 같은 결(다섯 판이 공용 로직을 각자 복사해 쓰는 관례, 루트 CLAUDE.md).
    /// 바닥/발판에 실제 PBR 재질(Poly Haven)을 씌울 때, 표면 크기에 맞는
    /// 반복 횟수로 다시 구운 인스턴스를 만든다(공유 안 함 — StoryTerrainBuilder.
    /// MakeMaterial(color)와 같은 결).
    /// </summary>
