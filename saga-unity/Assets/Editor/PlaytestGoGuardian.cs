...
namespace Saga.EditorTools
{
    ...
    /// 가까운 샷·넘김·한 번) · 토벌(금·경험·다시 안 섬·PlannedCount) · 세이브 v17 왕복·v14 로드. 끝나면 수호장·돈·경험·세이브를 되돌린다.
    ...
    public static class PlaytestGoGuardian
    {
        ...
        public static bool Run(string tag)
        {
            ...
            if (_ok) Debug.Log($"[{_tag}] guardian OK - 표·겉(뇌) 상성 깨짐·0.8초·속(화) 면역/물리/수 상성·3초 드러눕기·귀가 복구·등장 컷(브레인·18m·멈춤·HUD·이름표·포효·가까운 샷·넘김·한 번)·토벌(금·경험·안 섬)·세이브 v17/v14 |{metrics}");
            return _ok;
        }
        ...
        private static void CheckSave(FieldEnemy g)
        {
            ...
            if (!json.Contains("\"version\":17") || !json.Contains("\"guardianDown\":true")) Fail("세이브 v17 에 토벌이 안 남았다");
            GuardianState.Restore(false);
            if (!SaveState.TryLoad() || !GuardianState.Defeated) Fail("v17 왕복 뒤 토벌이 사라졌다");
            string v14 = json.Replace("\"version\":17", "\"version\":14").Replace(",\"guardianDown\":true", "");
