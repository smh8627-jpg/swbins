...
|---|---|---|---|
| ① | **들판 전투** | 아래 107-1 | 진행 |
| ② | **이동** | 점프(Space·모바일 버튼)·스태미나 100 공용(달리기·회피·등반·활공·수영)·등반(가파른 면, 꼭대기 넘어오르기)·활공(발밑 4m+)·수영(기력 0 → 마지막 땅). 등반·활공 전용 Mixamo 클립 | 대기 |
| ③ | **지역 지도** | 산을 막힌 벽(6m 상자)에서 오를 수 있는 고원·봉우리·절벽으로, 멀리 보이는 랜드마크, 순간이동 지점(역참), M 지도 화면(지역 이름·발견한 곳·순간이동) | 대기 |
| ④ | **보물 상자** | 등급 4 × 잠금 3(없음·무리 전멸·원소 석등), 다가가면 열림 | 대기 |
| ⑤ | **원소 쓰는 적** | 원소 방패(같은 원소 면역·물리 ×0.4·상성 ×2.5, 깨지면 비틀거림) | 대기 |
| ⑥ | **동료 모델** | 교체하면 몸이 바뀐다(지금은 Maria 하나에 원소 빛깔만) | 대기 |
...
- 진단: `PlaytestHeadless.CheckFieldCombat`.
