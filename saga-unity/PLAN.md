...
| DUNGEON | 5.1 축복 3택 5.2 유품(죽음 비용·회수) 5.3 부적 던전 티어 5.4 월드 보스 75초 5.5 난입 파도 5.6 목표판·카드 5.7 시대 퓨전 5.8 손맛 2차·가시화 | **5.8 → 5.1 → 5.2 → 5.3 전부 완료(2026-09-19)** — 다음은 5.4(월드 보스 75초). 5.7 은 웹 선행 결과 뒤. 5.8 중 hitstop·타격 VFX·죽음 표식 은 2026-09-17 완료 — 101-3 C·F 표 참고, **`LootMarker`(101-3 F, 적 사망 시각 잔향뿐)는 5.2(플레이어 죽음 비용·회수, 실제 골드 반환)와 다른 것 — 혼동 주의**. 5.1 은 웹판의 무예 4칸·서명 무예·7원소 시너지(36개 표)를 그대로 옮기지 않았다 — 이 트랙엔 원소 시스템도 다중 스킬 로드아웃도 없어, 대신 이 트랙 고유의 "하나를 크게(강공격) 대 여럿을 조금씩(회전베기)" 빌드(51장)를 직접 강화하는 공(攻)/수(守)/선(旋) 3축으로 재해석했다 — SagaGo `PerkState`(승급 3택)와 같은 구조, 새 `BlessingState`. 5.2 도 웹판의 "다음 회차에 그 층에 도달하면 회수"(런 리셋 전제)를 그대로 옮기지 않았다 — 이 트랙은 편도 진행(방을 뜨면 못 돌아옴)이라 "회수 전에 방을 뜨면 잃는다"로 좁혔다: 사망 시 소지 골드 전부를 그 자리 `GraveMarker`로 내려놓고 방을 뜨면 자동으로 사라진다. 사망 화면은 이 트랙 표준 `SessionCard`(101-2 B) 재사용, 세이브에는 안 남는다. 5.3 도 웹판의 "굴혈 앞 티어 1~10 선택 + 부적 인벤토리"(소모품 메타 시스템)를 그대로 옮기지 않았다 — 허브 자체가 없어(편도 진행) 선택지를 못 두니, "층 10 이후 보스층마다 자동으로 걸리는 변형자"로 좁혔다: `SigilState`가 층 번호만으로 결정적으로 정예 폭증(적 HP×2)·유리대포(주고받는 피해×1.5) 둘 중 하나를 고정해 적 스폰(`DungeonFloorRunner`)·`HeroState.HitDamage`에 배율로 얹고, 클리어(다음 층 진입)하면 그 층 두목 보상만큼 금을 더 준다(웹판 9개 변형자 풀도 원소·시야 시스템이 없어 둘로 좁혔다) | `PlayerCombat`·`DungeonEnemy`·`DungeonFloorRunner`·`HeroState`·`DamagePopup`·`HitSpark`·`LootMarker`·`GraveMarker`·`CameraRig`·`BlessingState`·`BlessingChoiceUi`·`SigilState` |
