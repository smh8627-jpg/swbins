...
옮겼다. 무기/방어구/성/장수 실제 이름, 퀘스트 목표 문장·고유명(예:
STORY "첫 사냥") 같은 **데이터 콘텐츠는 여전히 한국어 그대로** —
chrome(라벨)만 옮기는 경계를 계속 지켰다. 이걸로 새 코드가 짓는
UI 전부에서 chrome은 거의 다 옮겨졌고, 남은 한국어는 전부 진짜
번역이 필요한 데이터 콘텐츠(대사·퀘스트 서술·아이템/장수/도시
이름·REALM 문답)뿐이다.
