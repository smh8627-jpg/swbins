...
# 66-2. 아트 방향 — 사실적(포토리얼) PBR (2026-09-13, 정정됨)

## 결정 — saga-godot과 다른 방향으로 갈라섬
...
- 캐릭터/환경 모두 **표준 URP Lit(PBR: 알베도·메탈릭·러프니스·노멀맵)**
  머티리얼을 그대로 쓴다 — 셀셰이딩·아웃라인·밴드 명암 같은 카툰 전용
  기법은 필요 없다. 커스텀 셰이더를 새로 짤 필요가 (적어도 스타일화를
  위해서는) 없다는 뜻 — 이게 saga-godot 대비 이 트랙의 가장 큰 차이다.
- 텍스처·모델 디테일(고해상도 노멀맵, PBR 재질값 등)이 실제 화질을
  좌우한다(66-1장 8행이 이미 짚어 둔 원칙 — "에셋 품질이 엔진보다
  중요"). 다만 모바일 목표(45장)는 그대로라 텍스처 해상도·폴리곤 수는
  여전히 모바일 프로파일에서 조절 대상.
...
- 사실적 방향에 맞는 캐릭터·환경 에셋 소스 조사(무료/CC0 또는 라이선스
  확인된 사실적 PBR 킷)
- Kenney·VRoid 플레이스홀더를 실제 사실적 에셋으로 순차 교체(44장
  우선순위: Player → 주요 Enemy → Boss → Environment → Building → …)
- 66-1장 PC/Mobile 두 프로파일이 실제 사실적 에셋으로도 성능·화질
  균형이 맞는지 확인(카툰 방향보다 텍스처·라이팅 비용이 커질 수 있어
  45장 모바일 목표와 자주 대조할 것)
