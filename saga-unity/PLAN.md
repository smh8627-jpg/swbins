...
스타일 혼재 **해당**(Kenney 잔존·VRoid) · 후처리 **거의 완료** — SSAO·**LUT 5장 완료(2026-09-22, 각각 마을=따뜻/차가움 · 굴혈=청록 · 들판=황금시각 · 필드=고대비 · 성=저채도)**, Screen Space Shadows(새 Renderer Feature 추가 필요, GUID 위험 커 보류)만 미착수 · 그림자 계단 **Player는 완료(2026-09-22)** — Cascade 1은 유지(모바일 성능 목표, 102-2 원안)하되 `BlobShadow`로 접지 그림자 보완, 나머지 캐릭터(적·NPC)는 미착수 · 바닥 한 색 **해당**(`VertexColorLit` 정점색 지형) · 하늘·안개 **GO 외 해당** · 스케일 **해당**(Kenney 1.0 vs Mixamo 1.75 혼재) · 애니 끊김 **재조사 결과 해소(2026-09-22)** — Maria·Abe·Brute 세 컨트롤러(`Assets/Animators/*.controller`) 전부 `m_TransitionDuration` 0.1~0.15s 블렌드가 이미 있다(직접 YAML 확인, `AddReturnToIdle()`·`AddAnyStateTrigger()` 빌더 코드도 동일) — 이 표를 쓴 시점(초기 102장)보다 나중에 44장 Mixamo 교체가 블렌드까지 같이 넣었는데 표만 안 고쳐져 있었다. 게임 코드에 `Animator.Play/CrossFade` 직접 호출 없음(전부 `SetTrigger`/`SetFloat`라 선언된 duration이 그대로 적용) 확인 · 타격 반응 **해당**(101-1 C) · UI 폰트·패널 **일부**(`RealmUiKit`·`EncounterUiKit` 둘, 통일 안 됨) · 카메라 클리핑 **GO·DUNGEON 완료(2026-09-22)** — Cinemachine 패키지 없이(이 트랙은 카메라를 전부 수동 코루틴으로 다룬다, `CameraRig.cs` 클래스 주석) `CameraRig.ResolveCollisionZoom()`(raycast pull-in, `CameraSkin`으로 자기 CharacterController 오검출 방지)만 추가 — FOREST는 벽 충돌 없는 고정 카메라라 원래 스코프 밖(그대로), REALM 오빗은 미착수.
