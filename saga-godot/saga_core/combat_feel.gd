...
## ③ **피격 플래시** — target의 첫 `MeshInstance3D` 자식이 쓰는
##    `StandardMaterial3D.albedo_color`를 흰색으로 80ms 바꿨다 되돌린다.
##    PLAN 102-3이 계획한 진짜 셀셰이더 `hit_flash` uniform은 아직 그
##    셰이더가 DUNGEON 몬스터에 안 걸려 있어(`dungeon_enemy.gd::
##    _spawn_visual()`가 `StandardMaterial3D`를 직접 쓴다) 이번엔 그
##    재질을 직접 바꾸는 것으로 근사했다 — 102장이 셰이더를 입히면 이
##    자리만 uniform 대입으로 바꾸면 된다.
...
func _do_flash(target: Node3D) -> void:
	...
	if mesh != null and mesh.material_override is StandardMaterial3D:
		...
		if not _flash_state.has(key):
			_flash_state[key] = {"mesh": mesh, "orig": mat.albedo_color}
