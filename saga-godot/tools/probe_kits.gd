...
func _physics_process(_delta: float) -> void:
	...
	match _step:
		...
		1: # ① 표
			...
			var ok: bool = Kits.KITS.size() == 12 and is_equal_approx(float(_fc.call("skill_cd_of", "self")), 6.0) \
		...
		19: # ⑲ 이야기 동료 표
			...
			for c in Story.CHAPTERS:
				if Story.MEMBERS.has(String(c.get("join", ""))):
					joins += 1
			_check("story_members", ok and joins == 7, "members=%d joins=%d wanderer=%s/%s" % [Story.MEMBERS.size(), joins, Elements.element_of("story_wanderer"), Weapons.type_of("story_wanderer")])
			_next()
