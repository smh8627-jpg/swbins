...
func _physics_process(_delta: float) -> void:
	...
	match _step:
		0: # ① 개수·자리
			...
			_check("layout", StarShards.total() == 20 and regions.size() == 3 and placed and nodes.size() == 20 and StarShards.collected() == 0,
		...
		5: # ⑥ 다시 짓기
			...
			var ok := nodes.size() == 18 and is_equal_approx(float(_p.get("stamina_max")), 108.0)
