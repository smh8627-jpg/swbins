...
## 단순화한 것과 같은 결의 판단).
##
## **이번 걸음은 신수 11종 중 사신(四神, 청룡·백호·주작·현무) 넷만
## 세계에 배치한다** — pets.gd는 11종 데이터를 전부 옮겼지만, 나머지
## 일곱(삼족오·해태·구미호·도깨비·불가사리·홍염마·섬영마)은 다음 몫으로
## 남긴다(한 번에 다 안 채운다 — 토큰 절약 규칙 3). 사신은 원래 동서
## 남북을 지키는 신수라 지도 네 가장자리에 각각 세웠다.
##
