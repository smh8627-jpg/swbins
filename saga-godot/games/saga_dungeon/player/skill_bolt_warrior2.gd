...
## PLAN.md 51장 "장비→빌드" — 무장(warrior) br=5의 row2를 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_bolt.gd(s_wave)·skill_bolt_
## warrior.gd(w_throw, br4)와 계산이 완전히 같다(가장 가까운 적 하나를
## 즉시 맞히는 히트스캔). 무장의 두 번째 bolt(w_throw에 이어) —
## prereq_of()가 w_frostcleave(같은 br row1, 1점 필요)를 가리킨다.
## 이걸로 무장 br5가 3/3(w_intimidate·w_frostcleave·w_thunderlance)으로
## 찬다.
##
...
const SKILL_KEY := "w_thunderlance"
...
func _ready() -> void:
	add_to_group("skill_bolt_warrior2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_72"):
		try_cast()
...
func try_cast() -> bool:
	...
	_strike_thunderlance(target, sk, rank)
	return true
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(w_thunderlance는 el:'lit').
func _strike_thunderlance(enemy: Node, sk: Dictionary, rank: int) -> void:
