...
## PLAN.md 51장 "장비→빌드" — 도독(marshal)의 둘째 활성 무예로 "chain"
## 모양을 채운다(dungeon_skills.gd 헤더 참고, 원작 desc "기를 실어 가까운
## 적을 연달아 친다" 그대로). skill_chain.gd(a_chain)와 판정·감쇠 계산이
## 완전히 같다 — 원작 dungeon.js applyShapeSkill()의 'chain'은 shape 하나로
## 모든 chain 무예를 처리하기 때문이다. 다른 점은 SKILL_KEY·입력 액션·el
## 기본값(m_chain은 'chi', a_chain은 'phys')뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_chain_marshal")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_10"):
		try_cast()
...
## applyShapeSkill()의 "sk.el || 'phys'" 그대로, m_chain은 el:'chi').
