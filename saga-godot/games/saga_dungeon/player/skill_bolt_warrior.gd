...
## PLAN.md 51장 "장비→빌드" — 무장(warrior)의 넷째 활성 무예로 "bolt"
## 모양을 채운다(dungeon_skills.gd 헤더 참고). skill_bolt.gd(s_wave)와
## 계산이 완전히 같다(가장 가까운 적 하나를 즉시 맞히는 히트스캔). 다른
## 점은 SKILL_KEY·입력 액션·el 기본값(w_throw는 el 필드가 없어 'phys')뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_bolt_warrior")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_17"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(w_throw는 el 없음).
