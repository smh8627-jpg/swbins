...
## PLAN.md 51장 "장비→빌드" — 도독(marshal)의 셋째 활성 무예로 "swing"
## 모양을 채운다(dungeon_skills.gd 헤더 참고). skill_whirl.gd(w_whirl)와
## 계산이 완전히 같다(자기 둘레 반경 안의 적 전부를 때린다,
## `DungeonRunState.reach_mult()` 포함). 다른 점은 SKILL_KEY·입력 액션·el
## 기본값(m_smite는 el:'chi')뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_swing_marshal")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_13"):
		try_cast()
...
## skill_whirl.gd::_strike_whirl()과 같은 계산 — el 기본값만 다르다(m_smite는 el:'chi').
func _strike_smite(enemy: Node, sk: Dictionary, rank: int) -> void:
