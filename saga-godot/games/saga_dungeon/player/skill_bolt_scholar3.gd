...
## PLAN.md 51장 "장비→빌드" — 책사(scholar)의 br=1 갈래를 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_bolt.gd(s_wave, br2)·
## skill_bolt_scholar2.gd(s_fire, br0)와 계산이 완전히 같다 — 원작
## dungeon.js applyShapeSkill()의 'bolt'는 shape 하나로 모든 투사체
## 무예를 처리하기 때문이다. 책사가 bolt를 세 갈래(br2·br0·br1)에 갖는
## 첫 사례라 스크립트 이름은 `scholar3`. 이걸로 **다섯 직업 전부가
## 여섯 갈래(br0~5)에 row0을 갖는다**. 다른 점은 SKILL_KEY·입력 액션·
## el 기본값(s_ice는 el:'cold')뿐이다.
##
## 기력(mp)이 없어 sk.cost(16)는 소비하지 않는다(다른 무예와 같은 판단) —
## 쿨다운(sk.cd, 3초)만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "s_ice"
...
func _ready() -> void:
	add_to_group("skill_bolt_scholar3")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_26"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(s_ice는 el:'cold').
