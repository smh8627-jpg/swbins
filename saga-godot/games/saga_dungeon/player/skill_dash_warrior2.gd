...
## PLAN.md 51장 "장비→빌드" — 무장(warrior) br=1의 row1을 채우는 "dash"
## 모양(dungeon_skills.gd 헤더 참고). skill_dash_warrior.gd(w_dash, 같은
## br row0)와 계산이 완전히 같다 — prereq_of()가 w_dash(1점 필요)를
## 가리킨다. 무장의 두 번째 dash라 스크립트 이름은 `warrior2`.
##
## DASH_DURATION=0.36은 원작 "0.2 * (sk.far||1)"에 w_leap의 far(1.8)를
## 곱한 값(0.2*1.8) — dungeon_skills.gd 헤더의 dash 환산 그대로, 더 멀리
## (약 9m) 돌진한다. 기력(mp)이 없어 sk.cost(26)는 소비하지 않는다 —
## 쿨다운(sk.cd, 9초)만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "w_leap"
const DASH_SPEED := 620.0 * (6.0 / 148.0) # 웹 BASE_SPD(148px/s)→WALK_SPEED(6.0m/s) 환산
const DASH_DURATION := 0.2 * 1.8 # 원작 "0.2 * (sk.far||1)" — w_leap의 far=1.8
...
func _ready() -> void:
	add_to_group("skill_dash_warrior2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_37"):
		try_cast()
...
## skill_dash_warrior.gd::_strike_dash()와 같은 계산.
