...
const SKILL_KEY := "s_blink"
const DASH_SPEED := 620.0 * (6.0 / 148.0) # 웹 BASE_SPD(148px/s)→WALK_SPEED(6.0m/s) 환산
const DASH_DURATION := 0.2 # 원작 "0.2 * (sk.far||1)" — s_blink은 far 없음
...
func _ready() -> void:
	add_to_group("skill_dash_scholar")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_18"):
		try_cast()
...
## skill_dash.gd::_strike_dash()와 같은 계산 — el 기본값만 다르다(s_blink는 el:'lit').
