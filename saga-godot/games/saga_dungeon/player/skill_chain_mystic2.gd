...
## PLAN.md 51장 "장비→빌드" — 방사(mystic) br=5의 row2를 채우는 "chain"
## 모양(dungeon_skills.gd 헤더 참고). skill_chain.gd(a_chain)·skill_chain_
## mystic.gd(y_chain, br3)와 판정·감쇠 계산이 완전히 같다. 방사의 두 번째
## chain(y_chain에 이어) — prereq_of()가 y_hellstrike(같은 br row1, 1점
## 필요)를 가리킨다. 이걸로 방사 br5가 3/3(y_thunderdoom·y_hellstrike·
## y_frostchain)으로 찬다.
##
## 원작에 `r` 필드가 없어 skill_chain_mystic.gd와 같은 기본 탐색 반경
## (DEFAULT_RANGE=7.65) 그대로(y_chain과 같은 경계). 기력(mp)이 없어
## sk.cost(24)는 소비하지 않는다(다른 무예와 같은 판단) — 쿨다운(sk.cd,
## 9초)만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "y_frostchain"
...
func _ready() -> void:
	add_to_group("skill_chain_mystic2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_71"):
		try_cast()
...
func try_cast() -> bool:
	...
	while cur != null and hop < hops:
		_strike_frostchain(cur, sk, value * (1.0 - hop * FALLOFF_PER_HOP))
		hit.append(cur)
...
## skill_chain.gd::_strike_chain()과 같은 계산 — el 기본값만 다르다(y_frostchain은 el:'cold').
func _strike_frostchain(enemy: Node, sk: Dictionary, mult: float) -> void:
