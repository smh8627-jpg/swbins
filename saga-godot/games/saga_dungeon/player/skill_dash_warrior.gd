...
## PLAN.md 51장 "장비→빌드" — 무장(warrior)의 셋째 활성 무예로 "dash"
## 모양을 채운다(dungeon_skills.gd 헤더 참고). skill_dash.gd(a_dashshot)와
## 계산이 완전히 같다(속도·판정 반경 환산 근거도 그 파일·dungeon_skills.gd
## 헤더 참고). 다른 점은 SKILL_KEY·입력 액션뿐 — w_dash도 el 필드가 없어
## a_dashshot과 마찬가지로 "phys" 기본값을 쓴다.
##
...
const SKILL_KEY := "w_dash"
const DASH_SPEED := 620.0 * (6.0 / 148.0) # 웹 BASE_SPD(148px/s)→WALK_SPEED(6.0m/s) 환산
const DASH_DURATION := 0.2 # 원작 "0.2 * (sk.far||1)" — w_dash은 far 없음
...
func _ready() -> void:
	add_to_group("skill_dash_warrior")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_12"):
		try_cast()
...
## skill_dash.gd::_strike_dash()와 같은 계산.
