...
## PLAN.md 51장 "장비→빌드" — 책사(scholar) br=3의 row1을 채우는 "swing"
## 모양(dungeon_skills.gd 헤더 참고). skill_swing_archer.gd(a_flourish)와
## 계산이 완전히 같다(자기 둘레 반경 안의 적 전부, `DungeonRunState.
## reach_mult()` 포함). **책사의 첫 swing** — prereq_of()가 s_chainfire
## (같은 br row0, 1점 필요)를 가리킨다.
##
...
func _ready() -> void:
	add_to_group("skill_swing_scholar")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_48"):
		try_cast()


## HUD의 무예 버튼도 이 진입점을 그대로 부른다(skill_swing_archer.gd::try_cast()와 같은 경계).
func try_cast() -> bool:
...
## skill_swing_archer.gd::_strike_flourish()와 같은 계산 — el 기본값만 다르다(s_fan은 el:'cold').
func _strike_fan(enemy: Node, sk: Dictionary, rank: int) -> void:
