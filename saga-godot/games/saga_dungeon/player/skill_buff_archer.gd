...
## PLAN.md 51장 "장비→빌드" — 궁장(archer) br=4의 row1을 채우는 "buff"
## 모양(dungeon_skills.gd 헤더 참고). skill_buff.gd(m_rally)와 계산이
## 완전히 같다 — `DungeonRunState.add_temp_buff()`를 그대로 부른다.
## **궁장의 첫 buff** — prereq_of()가 a_flourish(같은 br row0, 1점 필요)를
## 가리킨다.
##
## 기력(mp)이 없어 sk.cost(28)는 소비하지 않는다(다른 무예와 같은 판단) —
## 쿨다운(sk.cd, 14초)만으로 남발을 막는다.

const SKILL_KEY := "a_speedy"

var _cooldown_left := 0.0


func _ready() -> void:
	add_to_group("skill_buff_archer")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_46"):
		try_cast()
