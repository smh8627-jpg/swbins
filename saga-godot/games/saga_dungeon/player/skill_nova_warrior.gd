...
## PLAN.md 51장 "장비→빌드" — 무장(warrior) br=0의 row2를 채우는 "nova"
## 모양(dungeon_skills.gd 헤더 참고). skill_nova.gd(y_thunderdoom)와
## 계산이 완전히 같다(자기 둘레 반경 안 적 전부, reach_mult() 안 곱함).
## **무장의 첫 nova** — prereq_of()가 w_cleave(같은 br row1, 1점 필요)를
## 가리킨다. 이걸로 무장 br0이 3/3(w_whirl·w_cleave·w_quake)으로 찬다.
##
## r=4.41은 원작 150px÷BASE_REACH(34px) — dungeon_skills.gd 헤더의 nova
## 환산(원작 r이 reachOf()를 안 곱하는 원시 픽셀 값이라 34로 나눈다) 그대로.
## 기력(mp)이 없어 sk.cost(36)는 소비하지 않는다(다른 무예와 같은 판단) —
## 쿨다운(sk.cd, 11초)만으로 남발을 막는다. 원작 넉백(kb:60)은 이 슬라이스에
## 넉백이 없어 값만 보존하고 안 쓴다(w_whirl·w_cleave와 같은 판단).

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "w_quake"
...
func _ready() -> void:
	add_to_group("skill_nova_warrior")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_33"):
		try_cast()
...
## skill_nova.gd::_strike_nova()와 같은 계산 — el 기본값만 다르다(w_quake는 el 없음, phys).
func _strike_quake(enemy: Node, sk: Dictionary, rank: int) -> void:
