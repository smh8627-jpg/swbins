...
## PLAN.md 51장 "장비→빌드" — 책사(scholar) br=5의 row1을 채우는 "nova"
## 모양(dungeon_skills.gd 헤더 참고). skill_nova.gd(y_thunderdoom)와
## 계산이 완전히 같다(자기 둘레 반경 안 적 전부, reach_mult() 안 곱함).
## 책사의 네 번째 nova(s_blaze·s_meteor·s_frost에 이어) — prereq_of()가
## s_restore(같은 br row0, 1점 필요)를 가리킨다.
##
## r=3.82는 y_thunderdoom·a_gale과 원작 r이 똑같이 130이라 같은 환산값이
## 그대로 나온다(dungeon_skills.gd 헤더 참고). 기력(mp)이 없어 sk.cost
## (32)는 소비하지 않는다(다른 무예와 같은 판단) — 쿨다운(sk.cd, 10초)
## 만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "s_plague"
...
func _ready() -> void:
	add_to_group("skill_nova_scholar4")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_68"):
		try_cast()
...
## skill_nova.gd::_strike_nova()와 같은 계산 — el 기본값만 다르다(s_plague는 el:'pois').
func _strike_plague(enemy: Node, sk: Dictionary, rank: int) -> void:
