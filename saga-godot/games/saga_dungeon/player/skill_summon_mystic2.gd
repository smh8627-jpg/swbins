...
## PLAN.md 51장 "장비→빌드" — 방사(mystic) br=0의 row1을 채우는 "summon"
## 모양(dungeon_skills.gd 헤더 참고). skill_summon.gd(y_shade, 같은 br
## row0)와 계산이 완전히 같다 — prereq_of()가 y_shade(1점 필요)를
## 가리킨다. 방사의 두 번째 summon이라 스크립트 이름은 `mystic2`.
##
## `str_mul`은 skill_summon.gd가 이미 `sk.get("str", 1.0)`으로 일반화해
## 뒀다(y_horde의 str:1.5 그대로 소비 — 새 필드 처리 없음). 기력(mp)이
## 없어 sk.cost(36)는 소비하지 않는다 — 쿨다운(sk.cd, 16초)만으로 남발을
## 막는다.

const DungeonMinion := preload("res://games/saga_dungeon/world/dungeon_minion.gd")
const SKILL_KEY := "y_horde"
...
func _ready() -> void:
	add_to_group("skill_summon_mystic2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_40"):
		try_cast()
