...
func _ready() -> void:
	add_to_group("skill_bolt_mystic")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_20"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(y_soulbolt는 el:'chi').
