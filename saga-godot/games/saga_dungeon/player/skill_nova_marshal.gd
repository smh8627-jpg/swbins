...
func _ready() -> void:
	add_to_group("skill_nova_marshal")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_39"):
		try_cast()


## HUD의 무예 버튼도 이 진입점을 그대로 부른다(skill_nova_warrior.gd::try_cast()와 같은 경계).
func try_cast() -> bool:
...
## skill_nova_warrior.gd::_strike_quake()와 같은 계산 — el 기본값만 다르다(m_ring은 el:'chi').
func _strike_ring(enemy: Node, sk: Dictionary, rank: int) -> void:
