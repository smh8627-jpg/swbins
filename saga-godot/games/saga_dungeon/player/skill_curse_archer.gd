...
## PLAN.md 51장 "장비→빌드" — 궁장(archer) br=3의 row2를 채우는 "curse"
## 모양(dungeon_skills.gd 헤더 참고). skill_curse.gd(w_intimidate)·skill_
## curse_mystic.gd(y_curse)와 계산이 완전히 같다(느려짐은 apply_elem_
## slow(), 저주는 apply_hex() — 둘 다 기존 것을 재사용, 새 상태 없음).
## **궁장의 첫 curse** — prereq_of()가 a_venom(같은 br row1, 1점 필요)을
## 가리킨다. 이걸로 궁장 br3이 3/3(a_chain·a_venom·a_cripple)으로 찬다.
##
## r=4.12는 원작 140px÷BASE_REACH(34px) — dungeon_skills.gd 헤더의 nova/
## curse 환산 그대로(m_ring과 같은 값). 기력(mp)이 없어 sk.cost(28)는
## 소비하지 않는다 — 쿨다운(sk.cd, 10초)만으로 남발을 막는다.

const SKILL_KEY := "a_cripple"
...
func _ready() -> void:
	add_to_group("skill_curse_archer")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_41"):
		try_cast()
