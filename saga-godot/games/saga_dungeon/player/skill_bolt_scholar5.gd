...
## PLAN.md 51장 "장비→빌드" — 책사(scholar) br=5의 row2를 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_bolt.gd(s_wave)와 계산이
## 완전히 같다(가장 가까운 적 하나를 즉시 맞히는 히트스캔). 책사의
## 다섯 번째 bolt(s_wave·s_fire·s_ice·s_bolt에 이어) — prereq_of()가
## s_plague(같은 br row1, 1점 필요)를 가리킨다. 이걸로 책사 br5가
## 3/3(s_restore·s_plague·s_venombolt)으로 찬다.
##
## 기력(mp)이 없어 sk.cost(18)는 소비하지 않는다(다른 무예와 같은 판단) —
## 쿨다운(sk.cd, 4초)만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "s_venombolt"
...
func _ready() -> void:
	add_to_group("skill_bolt_scholar5")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_73"):
		try_cast()
...
func try_cast() -> bool:
	...
	_strike_venombolt(target, sk, rank)
	return true
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(s_venombolt는 el:'pois').
func _strike_venombolt(enemy: Node, sk: Dictionary, rank: int) -> void:
