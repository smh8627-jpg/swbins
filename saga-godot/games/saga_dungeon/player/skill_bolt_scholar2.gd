...
## PLAN.md 51장 "장비→빌드" — 책사(scholar)의 br=0 갈래를 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_bolt.gd(s_wave, br2)와 계산이
## 완전히 같다 — 원작 dungeon.js applyShapeSkill()의 'bolt'는 shape
## 하나로 모든 투사체 무예를 처리하기 때문이다. 책사가 bolt를 두
## 갈래(br2 s_wave·br0 s_fire)에 갖는 첫 사례라 스크립트 이름에 `2`를
## 붙였다. 다른 점은 SKILL_KEY·입력 액션·el 기본값(s_fire는 el:'fire',
## s_wave는 el:'chi')뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_bolt_scholar2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_24"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(s_fire는 el:'fire').
