...
## PLAN.md 51장 "장비→빌드" — 무장(warrior)의 다섯째 활성 무예로 "chain"
## 모양을 채운다(dungeon_skills.gd 헤더 참고). skill_chain.gd(a_chain)·
## skill_chain_marshal.gd(m_chain)·skill_chain_scholar.gd(s_chainfire)와
## 판정·감쇠 계산이 완전히 같다 — chain이 이걸로 다섯 직업 전부가 공유하는
## 첫 모양이 된다. 다른 점은 SKILL_KEY·입력 액션·el 기본값(w_chain은
## el 없음 → phys)뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_chain_warrior")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_21"):
		try_cast()
...
## skill_chain.gd::_strike_chain()과 같은 계산 — el 기본값만 다르다(w_chain은 el 없음 → phys).
