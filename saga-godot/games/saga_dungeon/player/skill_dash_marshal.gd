...
## PLAN.md 51장 "장비→빌드" — 도독(marshal) br=3의 row2를 채우는 "dash"
## 모양(dungeon_skills.gd 헤더 참고). skill_dash.gd(a_dashshot)와 계산이
## 완전히 같다 — **도독의 첫 dash**. prereq_of()가 m_press(같은 br row1,
## 1점 필요)를 가리킨다. 이걸로 도독 br3이 3/3(m_chain·m_press·m_charge)
## 으로 찬다.
##
## 원작에 `far` 필드가 없어 기본 지속시간 0.2초(w_dash·a_dashshot과
...
const SKILL_KEY := "m_charge"
const DASH_SPEED := 620.0 * (6.0 / 148.0) # skill_dash.gd와 같은 환산
const DASH_DURATION := 0.2 # 원작 "0.2 * (sk.far||1)" — m_charge는 far 없음
...
func _ready() -> void:
	add_to_group("skill_dash_marshal")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_59"):
		try_cast()
...
## skill_dash.gd::_strike_dash()와 같은 계산 — el 기본값만 다르다(m_charge는 el:'chi').
