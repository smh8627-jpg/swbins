...
## PLAN.md 51장 "장비→빌드" — 방사(mystic) br=5의 row1을 채우는 "swing"
## 모양(dungeon_skills.gd 헤더 참고). skill_whirl.gd(w_whirl) 계열과
## 계산이 완전히 같다(자기 둘레 반경 안 적 전부, `DungeonRunState.
## reach_mult()` 포함). 방사의 두 번째 swing(y_ghoststrike에 이어) —
## prereq_of()가 y_thunderdoom(같은 br row0, 1점 필요)을 가리킨다.
##
## 원작 넉백(kb:22)은 이 슬라이스에 넉백이 없어 값만 보존하고 안 쓴다
## (w_palm과 같은 판단). 기력(mp)이 없어 sk.cost(24)는 소비하지 않는다
## (다른 무예와 같은 판단) — 쿨다운(sk.cd, 6초)만으로 남발을 막는다.

const MeleeAttack := preload("res://games/saga_dungeon/player/melee_attack.gd")
const SKILL_KEY := "y_hellstrike"
...
func _ready() -> void:
	add_to_group("skill_swing_mystic2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_66"):
		try_cast()
...
func try_cast() -> bool:
	...
	for enemy in targets:
		_strike_hellstrike(enemy, sk, rank)
	return true
...
## skill_whirl.gd::_strike_whirl()과 같은 계산 — el 기본값만 다르다(y_hellstrike는 el:'fire').
func _strike_hellstrike(enemy: Node, sk: Dictionary, rank: int) -> void:
