...
## PLAN.md 51장 "장비→빌드" — 궁장(archer) br=1의 row2를 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_bolt_archer3.gd(a_ice, 같은
## br row1)와 계산이 완전히 같다 — prereq_of()가 a_ice(1점 필요)를 가리킨다.
## 궁장의 네 번째 bolt라 스크립트 이름은 `archer4`. 다른 점은 SKILL_KEY·
## 입력 액션·el 기본값(a_storm은 el:'lit')뿐이다. 이걸로 궁장 br1이
## 3/3(a_fire·a_ice·a_storm)으로 찬다.
##
...
func _ready() -> void:
	add_to_group("skill_bolt_archer4")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_32"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(a_storm은 el:'lit').
