...
## PLAN.md 51장 "장비→빌드" — 무장(warrior) br=4의 row2를 채우는 "summon"
## 모양(dungeon_skills.gd 헤더 참고). skill_summon.gd(y_shade)와 계산이
## 완전히 같다 — **무장의 첫 summon**. prereq_of()가 w_regen(같은 br
## row1, 1점 필요)을 가리킨다. 이걸로 무장 br4가 3/3(w_throw·w_regen·
## w_hound)으로 찬다.
##
## 원작에 `str` 필드가 없어 skill_summon.gd와 같은 기본 배율(1.0)을
## 그대로 쓴다(y_shade·a_hawk와 같은 경계). 기력(mp)이 없어 sk.cost
## (34)는 소비하지 않는다(다른 무예와 같은 판단) — 쿨다운(sk.cd, 15초)
## 만으로 남발을 막는다.

const DungeonMinion := preload("res://games/saga_dungeon/world/dungeon_minion.gd")
const SKILL_KEY := "w_hound"
...
func _ready() -> void:
	add_to_group("skill_summon_warrior")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_62"):
		try_cast()
