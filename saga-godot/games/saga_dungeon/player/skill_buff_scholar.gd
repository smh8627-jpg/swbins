...
## PLAN.md 51장 "장비→빌드" — 책사(scholar) br=4의 row2를 채우는 "buff"
## 모양(dungeon_skills.gd 헤더 참고). skill_buff.gd(m_rally)와 계산이
## 완전히 같다 — `DungeonRunState.add_temp_buff()`를 그대로 부른다.
## **책사의 첫 buff** — prereq_of()가 s_hex(같은 br row1, 1점 필요)를
## 가리킨다. 이걸로 책사 br4가 3/3(s_blink·s_hex·s_insight)으로 찬다.
##
## 기력(mp)이 없어 sk.cost(32)는 소비하지 않는다(다른 무예와 같은 판단) —
## 쿨다운(sk.cd, 15초)만으로 남발을 막는다.

const SKILL_KEY := "s_insight"

var _cooldown_left := 0.0


func _ready() -> void:
	add_to_group("skill_buff_scholar")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_63"):
		try_cast()
