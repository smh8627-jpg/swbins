...
## PLAN.md 51장 "장비→빌드" — 도독(marshal) br=5의 row2를 채우는 "dash"
## 모양(dungeon_skills.gd 헤더 참고). skill_dash.gd(a_dashshot)·skill_
## dash_marshal.gd(m_charge, br3)와 계산이 완전히 같다. 도독의 두 번째
## dash(m_charge에 이어) — prereq_of()가 m_venomfield(같은 br row1,
## 1점 필요)를 가리킨다. 이걸로 도독 br5가 3/3(m_flamesaber·
## m_venomfield·m_frostcharge)으로 찬다.
##
...
const SKILL_KEY := "m_frostcharge"
const DASH_SPEED := 620.0 * (6.0 / 148.0) # skill_dash.gd와 같은 환산
const DASH_DURATION := 0.2 # 원작 "0.2 * (sk.far||1)" — m_frostcharge는 far 없음
...
func _ready() -> void:
	add_to_group("skill_dash_marshal2")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_74"):
		try_cast()
...
func _physics_process(delta: float) -> void:
	...
	for enemy in get_tree().get_nodes_in_group("dungeon_enemy"):
		...
		if enemy.global_position.distance_to(_player.global_position) <= HIT_RADIUS:
			...
			_strike_frostcharge(enemy)


## skill_dash.gd::_strike_dash()와 같은 계산 — el 기본값만 다르다(m_frostcharge는 el:'cold').
func _strike_frostcharge(enemy: Node) -> void:
