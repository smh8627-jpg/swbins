...
## PLAN.md 51장 "장비→빌드" — 궁장(archer) br=3의 row1을 채우는 "bolt"
## 모양(dungeon_skills.gd 헤더 참고). skill_chain.gd(a_chain, 같은 br
## row0)의 뒤를 잇는 row1 — prereq_of()가 a_chain(1점 필요)을 가리킨다.
## 궁장의 다섯 번째 bolt라 스크립트 이름은 `archer5`. 다른 점은
## SKILL_KEY·입력 액션·el 기본값(a_venom은 el:'pois')뿐이다.
##
...
func _ready() -> void:
	add_to_group("skill_bolt_archer5")


func _process(delta: float) -> void:
	_cooldown_left = maxf(0.0, _cooldown_left - delta)
	if Input.is_action_just_pressed("dungeon_skill_36"):
		try_cast()
...
## skill_bolt.gd::_strike_bolt()와 같은 계산 — el 기본값만 다르다(a_venom은 el:'pois').
