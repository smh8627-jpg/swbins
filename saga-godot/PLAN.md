...
| ⑨ | **지도 원신 일치** | 좌상단 원형 미니맵(보는 방향 부채꼴) · M 지도 화면(세 지역 한 장, 끌기·휠) · 순간이동 지점(가까이 가면 활성화 → 지도에서 눌러 이동, 도착하면 모두 회복) · 지역 경계를 넘으면 지역 이름 · 지역별 탐험도 % — 상세는 구현 뒤 이 칸에 | — |

**남은 격차(⑧⑨ 뒤 순서)**: 인물 육성(레벨·돌파·경험서·재화 — SAVE_VERSION 3) → 신상(스태미나 늘리기·회복)·수집 눈동자 → 원소 시야 → 원소 늘리기(바람·얼음·바위·풀과 확산·빙결·결정·개화 등) → 무기·성유물 → 요리·채집 특산물 → 일일 의뢰·비경·주간 보스 → 모험 등급·세계 등급(적 레벨). 이름·그림·수치는 이 판 것으로(오마주 규칙 그대로).
