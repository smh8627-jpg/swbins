...
- **대체 예정(2026-09-24 사용자 확정)**: 상용을 위해 VRoid·Mixamo 는 `tools/char-forge/`(Blender 헤드리스 + CC0 MakeHuman·Quaternius)로 바꾼다. 순서·기준은 그 README §7 — 이 장 아래 줄은 교체 전까지의 현재 상태다.
...
**남은 격차(㉛ 뒤 순서)**: 이야기 여섯째 장(북쪽 봉우리 너머 — 셋째 제단, 가면 쓴 자와 처음 맞섬) → 편성 더(명단 넷 고르기·빼기, 원신 파티 편성 화면) → 이야기 동료 더(촌장·사공 — 장 보상으로)(세상에 서 있거나 이야기로 만나는 인물이 늘 때, 갈래보다 한 단 위로). 이름·그림·수치는 이 판 것으로(오마주 규칙 그대로).
