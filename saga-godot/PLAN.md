...
- **Q-d 사람 몫 — Mixamo·VRoid만 남음(2026-09-20 Quaternius 쪽은 완결, PROJECT_STATE 09-23 오기 정정)**: 사람이 무료 Standard 버전을 직접 받아 Quaternius 68종(나무·덤불 22+바위 24+잔디·꽃 22) **전부** `assets/vegetation/`·`assets/rocks/`에 확보·임포트 검증·4팔레트 스냅까지 끝냈다(ASSET_GUIDE 09-20 여러 절). 남은 건 **씬 배치(5단계)뿐**이고 이건 "못 받아서"가 아니라 어디에 얼마나 박을지 102 그래픽 개편 사람 판단으로 미뤄 둔 것 — 코드로 막힌 게 아니라 배치 스코프를 사람이 정해야 열린다. **Mixamo 애니 다운로드 — 정정(2026-09-23): 로그인만 사람 몫, 검색·선택·다운로드는 자동**(루트 `tools/mixamo_automation/`, 2026-09-21 구축, idle 클립 검증 완료). **VRoid 조형(주역 몇 명)은 여전히 자동화 대안 없음** — 무료 대체 파이프라인(CharacterGen 등) 검토했으나 스타일·안정성 문제로 기각(2026-09-23), 사람 몫 유지.
