...
1. 캐릭터 하나(Player)에 카툰 셰이더 시험 적용 → GUI로 실제로 원신
   느낌이 나는지 확인(코드만으로 판단 못 한다, 루트 CLAUDE.md 실기
   확인 방침대로 이 결정 자체가 시각 판단이라 예외적으로 초기에 한 번
   봐야 한다) → 톤 확정.
2. 확정되면 Enemy/NPC/Boss까지 같은 셰이더 확장.
3. 아웃라인은 성능 영향이 커서 마지막에 별도로 검토(모바일 실기기
   프레임 확인 필수, 71장 테스트 기기).
4. 환경(지형/식생/건물) 머티리얼 톤 보정은 8장·66장 "그래픽 확장" 순서
   (Player→Enemy→Environment→...)를 그대로 따라 뒤로 미룬다.
...
`saga-unity/`는 이 결정과 **별개로 반영이 안 돼 있다**(그 폴더 PLAN.md
22~23장·66-1장은 여전히 PBR/URP Lit 가정만 적혀 있다). saga-unity 쪽도
같은 방향(카툰/셀셰이딩)으로 갈지는 **그 폴더 PLAN.md에 별도로 결정을
남길 것** — 0장 "개념만 참고, 코드는 공유 안 함" 원칙대로, 이 장의
개념(밴드 셰이딩·선택적 아웃라인·painterly 환경)만 옮기고 구현은
Unity Shader Graph로 새로 짠다. 이번 작업 범위 밖(사용자가 saga-godot
쪽만 먼저 고치라고 지시).

## 검증

- 이 장 자체는 문서 결정이라 헤드리스로 검증할 게 없다(project.godot·
  씬·스크립트 변경 없음).
