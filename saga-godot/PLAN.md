...
**GO ⑥ 75초 토벌 — 부위 조준 방식 결정(2026-09-20, 구 105장 Q-g)**: 20장 모바일 전투 UI의 기존 "Target" 버튼을 재활용해 누를 때마다 갑주→병장→기마 순으로 대상 부위를 순환 선택한다(탭 대상 방식은 모바일 터치 정밀도가 낮아 제외). 공격은 현재 선택된 부위에 적용. **구현은 아직 안 함** — GO 는 실기 확인 대기 목록에 이미 여러 항목이 걸려 있어 §8 원칙대로 새 시스템을 얹지 않고 결정만 기록.
