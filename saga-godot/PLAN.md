...
**남은 격차(㉘ 뒤 순서)**: 대화 연출 둘째 단계(인물 몸짓·입 모양, 지금은 카메라만) → 이야기 다섯째 장(서쪽 고개 너머 옛길 — 둘째 제단) → 고유 스킬 손으로 더(나그네처럼 이야기로 만나는 인물)(세상에 서 있거나 이야기로 만나는 인물이 늘 때, 갈래보다 한 단 위로). 이름·그림·수치는 이 판 것으로(오마주 규칙 그대로).
