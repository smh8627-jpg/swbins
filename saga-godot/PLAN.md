...
**남은 격차(⑪ 뒤 순서)**: 특성(스킬) 레벨·운명의 자리(육성 나머지) → 원소 시야 → 원소 늘리기(바람·얼음·바위·풀과 확산·빙결·결정·개화 등) → 무기·성유물 → 요리·채집 특산물 → 일일 의뢰·비경·주간 보스 → 모험 등급·세계 등급(적 레벨). 이름·그림·수치는 이 판 것으로(오마주 규칙 그대로).
