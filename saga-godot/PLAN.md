...
- **Q-h 102-1 스케일 표와 실제 세계 배율의 충돌**(2026-09-18 발견): 102-1 표는 "사람 1.7m"지만 지금 다섯 판(특히 승인받은 GO·DUNGEON·FOREST)은 캐릭터 임의 배율(1.25배 등)로 실측 키가 3.4m 안팎이고 카메라 거리·충돌 반경·지역 크기가 전부 그 키에 맞춰 튜닝돼 승인까지 났다. `GLBUtils.fit_height()`(신규, 아직 어디서도 안 부름)로 1.7m를 그대로 적용하면 승인받은 세 판의 체감이 반토막 난다. (a) 102-1 표 숫자를 지금 세계 배율(사람 ~3.4m)에 맞게 다시 잰다 (b) 표는 "새로 들이는 GLB부터" 기준이고 기존 승인판은 그대로 둔다 (c) 세계 전체를 1.7m 기준으로 다시 튜닝(카메라·충돌·지역 크기 전부)한다 — 중 무엇을 할지.
