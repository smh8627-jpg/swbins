...
**남은 격차(㉝ 뒤 순서)**: 이야기 일곱째 장(넷째 제단 — 포구, 검은 가면 둘째 대결) → 이야기 동료 더(촌장·사공 — 장 보상으로) → 편성 여러 벌(원신 파티 1~4 저장)(세상에 서 있거나 이야기로 만나는 인물이 늘 때, 갈래보다 한 단 위로). 이름·그림·수치는 이 판 것으로(오마주 규칙 그대로).
