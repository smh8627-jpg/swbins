...
|---|---|---|---|
| GO | `games/saga_go/world/TestVillage.tscn` | **승인** 2026-09-11(사용자 실기 "재미도 있음") | 사건 7+밤 2+굴·여울·폭포 · 역사 인물 조우 3(마을 2·폐허 1, 설득 3라운드) · 도감 5갈래+pet(신수 11 배치) · 동물 6 · 날씨·계절·TimeOfDay · 역참·사진 모드·소문 · 지역 3(마을 11×11·포구 9×9·폐허 7×7, REGIONS 레지스트리) · 저장/버전 · **101-4 ①일과판+마무리 카드**(목표판 3줄·저장 시 세션 요약 카드) · **101-2 ②승급 3택**(부대 레벨업마다 공/수/보 3택+거절) · **101-2 ③패배 비용·회수**(도적전 패배 시 경험치 15%(상한 15) 그 사건에 남고 10분 안에 그 사건을 다시 이기면 회수) |
...
- **2026-09-16, 한 세션에서 104장 Phase 0(①~⑤) 뒤 101-4 GO 후보 ①②③까지 이어서 완료**("사가고돗 이어해줘 묻지말고", 상세 경위는 HISTORY 날짜 항목):
  ...
  - **101-2 ②승급 3택**: `games/saga_go/data/perks.gd`(풀 12, 공/수/보 축, 축 중복 없이 3장). 웹은 인물별 rank up, 이 판은 인물별 랭크가 없어 **부대 레벨업**에 붙였다. `party_state.gd`에 `level_up` 신호·`perks`·특성 배율 `_recompute()`. 거절 보상은 웹의 재화 대신 경험치(이 판엔 재화 없음).
  - **101-2 ③패배 비용·회수**(표준 F): `games/saga_go/data/drop_state.gd`(autoload) — 도적전 패배(`dealt > 0.0`인 기존 경계 그대로) 시 경험치 15%(상한 15, 동시 3개)를 그 사건에 남기고, 10분 안에 그 사건을 다시 이기면 회수(`Time.get_unix_time_from_system()` 실시간 기준, 저장·재시작에도 안 깨짐). 웹의 "같은 자리로 돌아와 회수"를 "그 사건 노드를 다시 이겨서 회수"로 좁혔다(사건이 패배해도 안 지워지고 같은 자리에 남는 이 판 특성상 같은 뜻).
  - 세 후보 전부 `layer_box`/for-루프 클로저 안전성을 헤드리스 최소 재현으로 직접 확인, 각 자가진단 스크립트는 커밋 전 지웠다. 헤드리스 3회 회귀 통과(레벨업·패배는 경기 중 이벤트라 짧은 회귀 자체엔 안 걸림, md5 불변 — project.godot 의 DropState autoload 한 줄은 의도한 변경).
- 그 앞(이전 세션, 2026-09-16): GO 폐허에 3번째 역사 인물 조우, 포구 콘텐츠 5호(고래뼈), 96·97 재감사(고칠 것 없음).

## 다음 작업 (우선순위 — 상세는 PLAN 해당 장)

1. **실기 확인 몰아서**(아래 "실기 확인 대기") — 사용자 몫. 결과는 HISTORY 에 날짜 항목으로. 이번 세션의 목표판·마무리 카드·승급 3택·패배 비용 화면도 여기 추가됨.
2. **PLAN 101-2 GO ④사당 시련**: `hero_encounter.gd` 골격 재사용, 3분 방 — 101-2 순서대로 다음.
...
- **GO**: 포구 전체(늙은 어부·갈매기·게·표류물·조각배·고래뼈) · 폐허(지형·갈림길·옛 유물·"결사" 설득 3라운드) · 신수 11 조우 · 역참·사진 모드·소문 · 목표판 3줄(화면 위치·겹침 여부) · 저장 시 마무리 카드(문구·닫기) · 레벨업 승급 3택 카드(문구·손맛, 실제로 여러 번 레벨업해 카드 다양성 체감) · 도적전에서 실제로 져서 짐 드랍 문구 확인 후 10분 안에 다시 이겨 회수 체감
