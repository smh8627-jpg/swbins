...
사용자가 지시한 순서를 다 마쳤다 — 다음은 실기 확인이거나, 사용자가
다시 방향을 정하는 것(직업 5종·보스층·소켓 등 남은 "제외" 목록, 또는
GO의 남은 큰 시스템).
...
- **(2026-09-12⑱ 신규, DUNGEON — 장비 등급+접사) 잡졸을 잡았을 때 바닥에
  등급색(흰·파랑·노랑·초록·금갈색) 상자가 실제로 다르게 보이는지, 주웠을
  때 토스트("🎁 등급 · 이름 (옵션 줄)")가 읽기 편한지, 등급 높은 무기를
  주운 뒤 실제로 공격이 더 세게 느껴지는지(무력 flat이 즉시 데미지에
  더해진다).** 위 "완료 단계 (추가, 2026-09-12⑱)" 참고 — 잡졸을 여러
  마리 잡아 등급이 갈리는 걸 봐야 확실히 체감된다.
- **(2026-09-12⑰, DUNGEON — 여러 방 연결) 방 0에서 문을 나와
  복도를 지나 방 1로 걸어 들어가는 과정이 로딩 없이 자연스럽게
  이어지는지, 복도 폭이 방 벽 틈보다 살짝 좁아 생기는 턱이 시각적으로
  거슬리는지, 방 1의 더 세진 잡졸(HP 30 vs 방 0의 24)이 실제로 더
  버틴다고 체감되는지, 방 0을 깨고 나서 뜨는 "다음 방으로 향한다"
  토스트와 방 1을 깨고 뜨는 "이번 슬라이스는 여기까지" 토스트가 서로
  다르게 느껴지는지.** 위 "완료 단계 (추가, 2026-09-12⑰)" 참고.
- **(2026-09-12⑯, DUNGEON — 은사) 문으로 나갈 때 뜨는 3택 은사
