...
- **2026-09-16, 한 세션에서 104장 Phase 0(①~⑤) 뒤 101-4/101-2 GO 후보 ①~⑤까지 이어서 완료**("사가고돗 이어해줘 묻지말고" 세 번, 상세 경위는 HISTORY 날짜 항목들 — 이 아래는 요약뿐):
  ...
  - **101-2 ④사당 시련**(표준 A E F): `shrine_trial.gd` — `duel_rules.gd` 재사용, 파도 4개(90/120/170/230, 이전 파도 남은 시간을 다음 파도로 넘겨 전체 180초 예산 유지), 하루 3회, 실패 재입장 10분, 클리어 시 경험치+**미배치 인물을 `hero_encounter.gd`로 새로 인스턴스화**(설득 3라운드 개방). `duel_hud.gd`로 전투 UI 공용화(리팩터).
  - **101-2 ⑤봉수대**(표준 A E G): `beacon_tower.gd`(지역 3곳 각 1) — 웹의 미니맵 리빌을 미니맵이 없는 이 판에 맞춰 **그 지역 `codex_discoverable`(place 갈래) 미발견분 전부 즉시 발견**으로 옮겼다(104-5 그룹 재사용). 한 번만 보상.
  - 다섯 후보 전부 헤드리스 자가진단(임시, 커밋 전 지움)으로 실제 동작 확인, 3회 회귀 통과(GO md5는 매번 바뀜 — 새 로직이 당연히 바꿈, error/warn 0).
- 그 앞(이전 세션, 2026-09-16): GO 폐허에 3번째 역사 인물 조우, 포구 콘텐츠 5호(고래뼈), 96·97 재감사(고칠 것 없음).

## 다음 작업 (우선순위 — 상세는 PLAN 해당 장)

1. **실기 확인 몰아서**(아래 "실기 확인 대기") — 사용자 몫. 결과는 HISTORY 에 날짜 항목으로. 이번 세션의 목표판·마무리 카드·승급 3택·패배 비용·사당 시련·봉수대 화면도 여기 추가됨.
2. **PLAN 101-2 GO ⑥75초 토벌**: 부위 파괴·저스트 회피 — 101-2 GO 순서 마지막.
