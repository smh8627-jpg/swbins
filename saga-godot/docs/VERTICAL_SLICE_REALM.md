...
**다음 이어질 것** — `realm3d.js`(여러 성을 한 지도에 띄우는 월드맵)로
확장, 무장의 성 소속을 명령 실행에도 적용하는 나머지 절반(단, 위 막다른
골목을 풀 방법 — 예: 시작부터 무장을 성마다 하나씩 배치 — 이 먼저 정해져야
한다), 또는 전쟁/외교(war.js/diplo.js)·문답(quiz.js) — 어느 쪽이든 승인 후.
