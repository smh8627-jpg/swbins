...
| saga-unity | 두목 전용 몸 둘째 묶음 — DUNGEON 황건 살수 `Ninja`·층 주인 `Demon T Wiezzorek`·기계화 정찰병 `Alien Soldier`, STORY 황건 두목 `Morak` 대기·걷기(--inplace)·공격·피격·쓰러짐 (몸은 `--tpose --out Ninja`/`Demon`/`AlienSoldier`/`Morak`) | Ninja: `Fighting Idle`·`Sneak`·`Kick`·`Hit Reaction`·`Dying` / Demon: `Mutant`×4·`Hit Reaction` / AlienSoldier: `Standing Idle`·`Sneak`·`Punch`·`Hit Reaction`·`Dying` / Morak: `Standing Idle`·`Walking`·`Punch`·`Hit Reaction`·`Dying` | Ninja: `Male Fight Idle Boxing Stance`·`Sneaking Forward Quickly`·`Mma Side Kick`·`Hit Reaction`·`Dying With Front Impact To The Head And Fall On One Knee` / Demon: `Mutant Breathing Idle`·`Mutant Brutal Walk`·`Mutant Jump Attack To Idle`·`Hit Reaction`·`Mutant Dying Falling Onto Back` / AlienSoldier: `Standing Idle Looking Around`·`Male Ninja Sneak Walk`·`A Cross Punch`·`Hit Reaction`·`Dying Shot To The Chest Falling On Two Knees` / Morak: `Standing Idle`·`Walking With A Swagger`·`Hook Punch With The Rear Hand`·`Hit Reaction`·`Dying Front Head Impact To Two Knees` → `SetupNpcCharacterImports.SetupBossBodies` | 2026-09-24 |
...
| saga-unity | FOREST 세 시대(109-4) 마을 사람 — 과거 `Castle Guard 01`·`Knight D Pelegrini` / 현대 `Pete`·`Sophie` / 미래 `Uriel A Plotexia`·`Jennifer` 대기·걷기(--inplace) (몸은 `--tpose --out CastleGuard`/`Pelegrini`/`Pete`/`Sophie`/`Uriel`/`Jennifer`). **`Pirate By P. Konstantinov` 는 의족이라 `LeftFoot` 이 없어 휴머노이드 실패** — 쓰지 말 것. 카드 모습은 `.product.product-character` 요소 스크린샷을 한 장으로 모아 보면 빠르다 | 남자: `Idle`·`Male Walk` · 여자(Sophie·Jennifer): `Idle`·`Female Walk` | 남자 `Weight Shift Idle`·`Male Standard Walk` / 여자 `Happy Idle Variation 1`·`Female Normal Walk` → `SetupNpcCharacterImports.SetupForestEraBodies` | 2026-09-25 |
| saga-unity | GO 인물 105 몸(109-7) — 여 `Kachujin G Rosales`·`Arissa`·`Eve By J.Gonzales`(--charquery eve) / 남 `Dreyar By M.Aure`(--charquery dreyar)·`Castle Guard 02`·`Heraklios By A. Dizon`(--charquery heraklios)·`Brady`(파일 Ch40_nonPBR)·`Joe`(파일 Ch33_nonPBR) 대기만 (몸은 `--tpose --out Kachujin`/`Arissa`/`Eve`/`Dreyar`/`CastleGuard02`/`Heraklios`/`Brady`/`Joe`). 동작은 주인공 Maria.controller 리타깃이라 대기 하나로 된다. 카드 108장 전부 모아 본 결과 역사풍 사람 몸은 이것뿐(`Akai E Espiritu`·`Erika Archer` 는 지금 Archer 와 거의 같고 `Girlscout`·`Vampire`·`Medea` 는 괴물·미래풍) | 남자 `Idle` · 여자 `Idle` | 남자 `Weight Shift Idle`(파일 Idle) · 여자 `Happy Idle Variation 1`(파일 Happy Idle) → `SetupNpcCharacterImports.SetupHeroBodies` | 2026-09-25 |
