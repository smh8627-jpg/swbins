...
> 상태: **saga-godot 몸 = VRoid 직접 디자인으로 확정(D4, 2026-09-25)**. 공방 툰 몸(`_cmp_*`)은 얼굴이 원신풍 애니가 아니라 쓰지 않는다. 이 도구는 VRoid 주역에 CC0 동작·얼굴 굽기·게임 배선을 맡는다(§10 `vroid_intake.sh`). saga-unity(사실풍)는 단계 3(MakeHuman 사실 몸)으로 이어 간다.
