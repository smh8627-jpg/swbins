...
- Q1. 유료판(Pro/Source, CC0)을 살까? 무료판에 빠진 동작을 직접 키프레임으로 짜는 비용과 저울질한다.
- Q2. 툰 얼굴은 어디를 기준으로 할까 — 지금 VRoid 얼굴 느낌에 맞출지, 새 얼굴로 갈지.
- Q3. Unity 는 FBX 로 갈까, glTFast 로 갈까. 지금 코드는 `MixamoRigUtil`(FBX Humanoid)에 맞춰져 있어 FBX 가 싸다.
