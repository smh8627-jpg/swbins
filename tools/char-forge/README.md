...
| `keyframes.py` | **자체 키프레임 동작**(`CF_*`) — UAL 무료판에 없는 동작을 동작 팩 뼈대 위에 코드로 짓는다. 자세 = 골반 이동·돌기(`yaw`)·등뼈·손발 방향(`dirs`) + 손목·발목 자리(`ik`, 두 마디 IK·팔꿈치/무릎 `pole`). `retarget`·`verify.py` 가 UAL 동작과 똑같이 받는다(레시피 `"climb": "CF_Climb_Loop"`). 지금: 등반·활공·방패 막기·막기 중 피격·활 대기·활 쏘기·치유 시전·무릎 꿇기(포로) |
...
| 4 ⏸ | 인물 명단 → 레시피 대량 생성(도감 `id` 마다) | 인물마다 실루엣이 다르다(키·체격·머리·옷 네 축 중 둘 이상) — **도구만 끝·보류(2026-09-26)**: `gen_hero_recipes.py`(saga-unity GO 도감 105, 두 축 미만 쌍 0 · 키 5칸·체격 4·머리 18·옷 22) + `calib_height.py`(목표 키 ±0.3%) → 105벌 빌드 실패 0(로컬 `_out/hero/`, 커밋 안 함). 눈으로 보니 옷이 문턱 미달(§8-1 09-26)이라 게임엔 안 넣는다. 옷 방식이 바뀌면 생성기의 `outfit_parts` 만 갈아 다시 뽑는다 · **다음 세션 순서(09-26)**: ① `garments.py` 옷 더(일본 갑옷·기모노, 삼국 장수 갑옷, 관모·복두) ② 같은 틀 여러 색(세력 색) ③ `gen_hero_recipes.py` 의 `outfit_parts` 를 껍데기 대신 진짜 옷(CC0 `mh_*` + `cf_*`)으로 → 105 다시 뽑기 ④ `render/` 로 Mixamo 와 비교해 사용자 판정 받기 |
