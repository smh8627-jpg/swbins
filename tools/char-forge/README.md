...
괴물 레시피 칸(`_cmp_real_goblin_01`·`_cmp_real_brute_01`):
...
- `kitbash` [부품] — 몸을 다 지은 뒤 실제 살을 재서 붙이는 자체 생성 부품. `horns`(머리 뼈, `length`·`radius`·`curl`·`color`) · `loincloth`(허리 → 허벅지 `length`, 위는 골반·아래로 갈수록 허벅지 무게 — 단면은 몸통·다리 살만 재야 한다: A 자세 손이 엉덩이 높이라 손까지 두르던 적이 있다). 재질 칸 이름은 부품 이름(`horn`·`cloth`), 그림 없이 바탕색 — Unity 빌더가 색을 옮긴다.
