...
' | grep @)` (26분) → 105벌 `build_real.py` 루프(50분). **그다음 할 수 있는 일**: 껍데기 남은 여섯(동쪽 윗옷 `tunic` 둘 jp_mitsukuni·kr_jeongmongju, 서쪽 짧은 옷 `robe_short` 둘 eu_michelangelo·wd_ibnsina, 부족 허리 천 둘은 그대로 둬도 됨) · 긴 옷 뒤 불룩(천 시뮬레이션 없이는 어렵다 — 가랑이 아래 좌우 무게 평균은 실패) · 동쪽 남자 짧은 머리(short01~04)를 상투·망건으로 |
