...
(function (global) {
  ...
  function stats(id) {
    ...
    if (pet && pet.bonus) {
      ...
      /* 연성(growth.js)이 올려 둔 만큼이 여기로 들어온다. 그 모듈이 없으면
         원래 값이 쓰인다 — 이 판에만 있는 축이므로 없어도 돌아야 한다 */
      out[k] += global.DG.growth ? global.DG.growth.bonusOf(pet) : pet.bonus.value;
    }
    return out;
  }
  ...
  function rankUpCheck(id) {
    ...
    /* 지역 특산물(cooking.js, PLAN §5 ⑲-6) — 오픈월드 RPG 승급 재료 문법. 그 모듈이 없으면 예전 그대로 */
  ...
  global.DG.hero = {
    ...
    gainExp: gainExp, awardParty: awardParty,
