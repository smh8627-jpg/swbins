...
| `editor/brief.js` | 게임 설명서(글) → project.json. 규칙만(API·인터넷 없음) — 편집기 📝 글로 · 서버 `/api/brief` · 시험이 같이 쓴다 |
...
편집기 **📝 글로**(또는 프로젝트 창 "📝 글로 만들기") — 한 줄에 하나씩 쓰면 규칙으로 판을 짓는다. 유료 API 는 쓰지 않는다.

```
제목: 불의 동굴
전투: 젤다식            (간단 · 원신식 · 젤다식 · 파판식 · 없음)
시점: 3인칭             (1인칭 · 3인칭 · 쿼터뷰 · 옆 · 고정)
장면: 들녘 마을
  나무 14
  주민: 어르신 "어서 오게" "두목을 부탁하네"
  상점: 잡화점 (포션 10, 씨앗 3)
  적: 들개 ×3 (체력 3)
  문 → 어둑한 동굴
장면: 어둑한 동굴
...
- 형식 전체는 창의 "형식 보기"(= `brief.js` `HELP`). 종류 서른다섯(자연·건물·줍기·채집·상자·사람·상점·동료·동물·적·보스·파도·던전·문·결승·장치·영지…).
- 자리는 씨앗 흩기(시작 곁·곳곳·먼 쪽·가장자리·북쪽 끝) — **같은 글이면 늘 같은 판**. 만든 뒤 편집기에서 고친다.
- 목표(모으기·쓰러뜨리기·모두 쓰러뜨리기·만나기·버티기·레벨·결승)는 표식 변수 `goalN`·`goalsDone` 이벤트로 바뀐다. 여럿이면 다 이뤄야 이기고, 결승이 있으면 그때 나타난다.
- 못 알아들은 줄은 줄 번호와 함께 알림으로 보이고 판은 그대로 짓는다. 실명 가드는 저장 때 걸린다.
- **📋 AI 안내문 복사** — 형식·예시가 든 안내문을 무료 채팅 AI(웹)에 붙여 넣고 맨 끝에 원하는 게임을 적으면 이 형식으로 써 준다. 받은 글을 창에 붙여 넣는다.
...
JSDOM_DIR=<jsdom 설치 폴더> node saga-web/tools/engine/test/dom.mjs   # 편집기·글로 만들기·실행기(틀 + 글로 만든 판)·내보낸 판을 jsdom 에서 → RESULT n/n
```
