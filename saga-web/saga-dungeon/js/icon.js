 ...
 * ---------------------------------------------------------------
 * 사가고(saga-go)의 `js/icon.js` 를 그대로 옮겼다(2026-09-08, "사가블로·
 * 사가스토리 UI 가 사가고랑 완전히 같지 않다" 제보) — 도구줄·독·지갑의
 * 이모지는 폰트가 그리는 것이라 기기마다 다르게 보인다. 사용자 방침
 * ("스크립트로 그리는 것은 다 에셋으로", 2026-08-28)에 따라 실제 아이콘
 * (Lucide, **ISC**)으로 바꾼다. 이 판에서 실제로 쓰는 이름만 추렸다 —
 * 나머지는 사가고 쪽 `icon.js`를 본다.
 *
 ...
 *
 * 소리(`btn-sound`) 버튼은 **일부러 안 건드렸다** — 상태가 바뀔 때마다
 * `textContent` 를 통째로 다시 쓰는 자리라 data-icon 을 달아도 다음 클릭에서
 * 곧바로 지워진다(사가고도 실은 이 버튼만 이 흠을 그대로 갖고 있다 — 거기
 * 맞춘 것이지 새 흠이 아니다).
 */
(function (global) {
  'use strict';
