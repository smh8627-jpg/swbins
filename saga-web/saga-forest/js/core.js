...
(function (global) {
  ...
  var SAVE_BASE = 'yeoksa-village/save';
  ...
  function freshSave() {
    return {
      ...
      auto: {                             // 자동 마을살이 (auto.js) — 켠 상태도 기억한다
        on: false, gather: true, errand: true, sell: true, review: true
      },
   ...
   *
   * 이 판에서 가장 값진 손잡이는 **때(계절·시간대·날씨·행사·날짜)** 다.
   * 마을이 실제 시계에 크게 기대므로, 그날로 가 보는 것이 곧 검증이다.
   */
  var TUNE_KEY = 'yeoksa-village/tune';
  var tuneCache = null;
  ...
  var POKE_KEY = 'yeoksa-village/admin/poke';
