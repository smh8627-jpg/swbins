...
(function (global) {
  ...
  function heightAt(x, z, seed, W, H) {
    ...
    /* 굵은 물결 하나에 잔물결 하나 — 하나만 쓰면 규칙적인 골이 진다.
       2026-09-07 — 마을 지형 창이 플레이어를 따라가게 되면서([[§57]])
       스폰에서 아주 멀리(out→1) 걸어 나가는 일이 실제로 흔해졌는데, 거기서
       이 진폭(190·46)이 그대로면 이웃한 조각(CHUNK=200칸)끼리 높이차가
       200에 육박해 "네모난 조각이 계단처럼 솟았다 꺼졌다" 하는 것으로
       보인다(사용자 실기기 제보) — 조각 하나가 통짜 평판이라 부드럽게
       기울지 않고 뚝뚝 끊겨서다. 굴곡 자체를 없애진 않되(4절 취지는
       살린다) 진폭을 1/4로 낮춰 계단 낙차를 눈에 덜 띄게 줄였다. */
    var big = mix(Math.floor(x / 260) + seed, Math.floor(z / 260) - seed);
    var fine = mix(Math.floor(x / 90) * 7 + seed, Math.floor(z / 90) * 13 - seed);
    return out * out * ((big - 0.35) * 46 + (fine - 0.5) * 11);
  }
  ...
  function chunkAt(cx, cz, seed, ring, dens, theme) {
    ...
    if (kind === 'forest') {
      ...
      for (i = 0; i < n; i++) {
        var r = spot(i + 10);
        /* 2026-09-07 — 실기기 제보("큰 돌로 화면이 안 보인다") 이후 s 상한을
           2.4→1.6으로 낮췄다(§57 창-추적 이후로 이 자리를 실제로 밟는 일이
           늘어 눈에 띄게 됐다) */
        out.push({ t: 'rock', x: r.x, z: r.z, s: 0.7 + rnd(i + 90) * 0.9,
                   rot: rnd(i + 20) * 6.28, h: 16 + rnd(i + 30) * 24 });
      }
    } else if (kind === 'ruin') {
