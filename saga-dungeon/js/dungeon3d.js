...
(function (global) {
   ...
   *  §28-5(2026-09-06) — 부감각 = atan((1-tl*0.55)/tl). 기존 0.62(≈47°)는
   *  3인칭 체이스캠에 가까웠다 — 0.4(≈62.9°)로 낮춰(=각도는 올려) 디아블로류
   *  부감(보통 60~75°)에 맞춘다. */
  function TILT() { return tuned('dg3d.tilt', 0.4); }
  ...
  /** PerspectiveCamera FOV(도) — §28-5(2026-09-06) 46°→34°, 디아블로류 아이소
   *  느낌을 내려고 원근왜곡을 줄였다. camAim() 의 DUNGEON_DIST_MUL 이 이 값에
   *  맞물려 있다 — 여기만 바꾸면 방이 화면 밖으로 나간다, 같이 바꿀 것. */
  var FOV_DEG = 34;
  /** 던전 쪽 camAim() dist 배수 — 예전 FOV(46°)에서 맞춘 1.05를, 좁아진
   *  FOV_DEG 에서도 같은 폭(방 대각선)이 화면에 담기도록 tan() 비율로
   *  다시 계산한다(손으로 반올림하지 않는다 — §28-5 설계안이 명시).
   *  화면에 담기는 폭 ∝ dist·tan(fov/2) 이므로 비율은 tan 의 역수다. */
  var DUNGEON_DIST_MUL = 1.05 *
    Math.tan(46 / 2 * Math.PI / 180) / Math.tan(FOV_DEG / 2 * Math.PI / 180);
  ...
  function camAim(px, py, W, H, zoom, tilt, close) {
    ...
    var tl = tilt === undefined ? 0.4 : tilt;
    /* 방 대각선을 화면에 담을 거리 — 방이 커지면 저절로 물러난다.
       **계수를 눈으로 맞췄다**: 화면에 담기는 세로는 대략 2·dist·tan(fov/2) 다.
       fov 46° 였을 때는 0.85·dist, 던전 쪽 dist 배수 1.05 로 방 대각선(666)을
       담았다(0.62 배수로 뒀다가 방이 화면 밖으로 나가 어둠만 찍혔던 적이 있다).
       **§28-5(2026-09-06) — FOV 를 46°→34°로 좁혔다**(디아블로류 아이소 느낌).
       화면에 담기는 폭은 dist·tan(fov/2) 에 비례하므로, FOV 가 좁아진 만큼
       dist 배수를 올려야 "방이 다 담긴다"는 불변식이 유지된다 — 아래
       DUNGEON_DIST_MUL 이 그 보정값을 tan() 으로 직접 계산해 둔 것이지 손으로
       굴린 숫자가 아니다.
       **`close`(마을 전용, 2026-09-02) — 사가의숲 쿼터뷰만큼 가깝게 해 달라는
       요청.** 던전 방은 벽 밖이 어둠뿐이라 위 실패가 그대로 재현되지만,
       마을·필드는 담이 없는 열린 땅이라 화면 밖으로 나가도 그냥 덜 보일 뿐이다
       — 그래서 마을에서만 훨씬 당겨 본다. 마을 쪽 0.4 는 FOV 를 좁혀도 그대로
       둔다(열린 땅이라 다 담을 필요가 없다, §28-5 설계안이 명시) */
