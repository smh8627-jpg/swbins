...
**2026-09-10, 이 저장소 자체의 폴더명을 `C:\saga`에서 `C:\swbins`로 바꾸기로
했다**(git 리모트 이름 `swbins`와 로컬 폴더명을 맞추려는 목적). 예전에 개인
도구 저장소가 쓰던 `C:\swbins` 경로는 그 전에 이미 `C:\swbins2`로 옮겨져
있었어서 겹치지 않는다. **아직 실제 rename은 안 됐다** — 세션이 이 폴더를
cwd로 잠그고 있어 `ren`이 "다른 프로세스가 사용 중" 오류로 실패한다. 사용자가
모든 세션을 닫은 뒤 직접 rename할 예정이다. 다음 세션이 `C:\saga`에서 열렸다면
아직 rename 전이라는 뜻이고, `C:\swbins`에서 열렸다면 rename이 끝난 것이니 이
문단을 정리한다.
